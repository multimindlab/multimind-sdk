...
from ..core.provider import EmbeddingResult, GenerationResult, ImageAnalysisResult
...
class PipelineStage(Generic[T, R]):
    ...
    def __init__(
        self, stage_type: StageType, handler: Callable[[T], R], config: Optional[StageConfig] = None
    ):
            ...
            try:
                if self.config.timeout:
                    result = await asyncio.wait_for(
                        self.handler(input_data), timeout=self.config.timeout
                    )
                else:
...
class Pipeline:
    ...
    def stage(
        self, stage_type: StageType, handler: Optional[Callable] = None, **config
    ) -> "Pipeline":
    ...
    async def _default_embed_handler(self, input_data: str) -> EmbeddingResult:
        ...
        return await self.router.route(
            TaskType.EMBEDDINGS, input_data, model="text-embedding-ada-002"
        )
    ...
    async def _default_analyze_handler(self, input_data: bytes) -> ImageAnalysisResult:
        ...
        return await self.router.route(
            TaskType.IMAGE_ANALYSIS, input_data, prompt="Analyze this image in detail."
        )
...
class PipelineBuilder:
    ...
    def qa_retrieval(self) -> Pipeline:
        ...
        return (
            pipeline.stage(StageType.EMBED, provider="openai", model="text-embedding-ada-002")
            .stage(StageType.RETRIEVE, parameters={"top_k": 5})
            .stage(
                StageType.GENERATE,
                provider="claude",
    ...
    def code_review(self) -> Pipeline:
        ...
        pipeline = Pipeline(self.router)
        return pipeline.stage(StageType.ANALYZE, provider="openai", model="gpt-4").stage(
            StageType.GENERATE,
            provider="claude",
            model="claude-3-sonnet",
    ...
    def image_analysis(self) -> Pipeline:
        ...
        return pipeline.stage(
            StageType.ANALYZE, provider="openai", model="gpt-4-vision-preview"
        ).stage(
            StageType.GENERATE,
            provider="claude",
            model="claude-3-sonnet",
    ...
    def text_summarization(self) -> Pipeline:
        ...
        ).stage(StageType.TRANSFORM, parameters={"format": "bullet_points"})
    ...
    def content_generation(self) -> Pipeline:
        ...
        return (
            ...
            .stage(StageType.TRANSFORM, parameters={"format": "markdown"})
            .stage(
                StageType.GENERATE,
                provider="openai",
                model="gpt-4",
    ...
    def data_analysis(self) -> Pipeline:
        ...
        return (
            ...
            .stage(StageType.TRANSFORM, parameters={"format": "json"})
        )
    ...
    def multi_modal_qa(self) -> Pipeline:
        ...
        pipeline = Pipeline(self.router)
        return (
            pipeline.stage(StageType.ANALYZE, provider="openai", model="gpt-4-vision-preview")
            .stage(StageType.EMBED, provider="openai", model="text-embedding-ada-002")
            .stage(StageType.RETRIEVE, parameters={"top_k": 3})
            .stage(
                StageType.GENERATE,
                provider="claude",
                model="claude-3-sonnet",
    ...
    def code_generation(self) -> Pipeline:
        ...
        return (
            ...
            .stage(StageType.TRANSFORM, parameters={"format": "python"})
            .stage(
                StageType.GENERATE,
                provider="claude",
                model="claude-3-sonnet",
    ...
    def translation_pipeline(self) -> Pipeline:
        ...
        return (
            ...
            .stage(StageType.TRANSFORM, parameters={"format": "text"})
            .stage(
                StageType.GENERATE,
                provider="openai",
                model="gpt-3.5-turbo",
    ...
    def research_assistant(self) -> Pipeline:
        ...
        return (
            pipeline.stage(StageType.EMBED, provider="openai", model="text-embedding-ada-002")
            .stage(StageType.RETRIEVE, parameters={"top_k": 10})
            .stage(
                StageType.ANALYZE,
                provider="claude",
                model="claude-3-sonnet",
