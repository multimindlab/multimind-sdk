...
class AdaptiveRouter(ModelRouter):
    ...
    async def select_model(
        self, task_type: str, input_data: Dict[str, Any], **kwargs
    ) -> Tuple[str, BaseLLM]:
        ...
        for model_id, model in self.providers.items():
            score = await self._calculate_model_score(model_id, task_type, input_data, task_metrics)
    ...
    async def _calculate_model_score(
        ...
        success_rate = model_metrics["successful_requests"] / max(
            1, model_metrics["total_requests"]
        )
        ...
        if self.importance_scorer:
            importance_result = await self.importance_scorer.score(
                str(input_data), {"timestamp": datetime.now().isoformat()}, task_type
            )
    ...
    async def record_performance(
        ...
        model_metrics["task_performance"][task_type] = 0.9 * model_metrics["task_performance"][
            task_type
        ...
        task_metrics["model_performance"][model_id] = 0.9 * task_metrics["model_performance"][
            model_id
        ] + 0.1 * (1.0 if success else 0.0)
    ...
    async def _adapt_routing_strategy(self) -> None:
        ...
        if hasattr(self.default_strategy, "weights"):
            ...
            total_requests = sum(
                len([p for p in recent_performance if p["model_id"] == model_id])
                for model_id in self.providers
            )
            ...
            for model_id in self.providers:
                if model_id in success_rates:
                    model_requests = len(
                        [p for p in recent_performance if p["model_id"] == model_id]
                    )
                    ...
                    ) * current_weight + self.adaptation_rate * performance_score
