...
from multimind.context_transfer import AdapterFactory, ContextTransferManager
...
Examples:
  ...
  multimind context-transfer --model_info deepseek
    ...
    transfer_group.add_argument(
        "--from_model", help="Source model name (e.g., chatgpt, claude, deepseek)"
    )

    transfer_group.add_argument(
        "--to_model", help="Target model name (e.g., deepseek, claude, gemini)"
    )

    transfer_group.add_argument(
        "--input_file", help="Path to input file containing conversation history"
    )

    transfer_group.add_argument("--output_file", help="Path to output file for formatted prompt")
    ...
    advanced_group.add_argument(
        "--include_safety", action="store_true", help="Include safety and ethical considerations"
    )

    advanced_group.add_argument(
        "--include_creativity", action="store_true", help="Include creativity instructions"
    )

    advanced_group.add_argument(
        "--include_examples", action="store_true", help="Include instruction to provide examples"
    )
    ...
    info_group.add_argument("--model_info", help="Show detailed information about a specific model")
    ...
    if not all(
        [
            parsed_args.from_model,
            parsed_args.to_model,
    ...
    try:
        ...
        try:
            ...
            print("\n📊 Transfer Summary:")
            print(
                f"   From: {from_model} ({from_caps.get('max_context_length', 'Unknown'):,} tokens)"
            )
