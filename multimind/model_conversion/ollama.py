...
class OllamaConverter(BaseModelConverter):
    ...
    def convert(
        ...
        model_config = {"name": os.path.basename(output_path), "path": model_path, **(config or {})}
        ...
        response = requests.post(f"{self.ollama_host}/api/create", json=model_config)
    ...
    def get_metadata(self, model_path: str) -> Dict[str, Any]:
        ...
        try:
            response = requests.get(
                f"{self.ollama_host}/api/show", json={"name": os.path.basename(model_path)}
            )
