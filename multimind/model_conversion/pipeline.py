...
class ConversionPipeline:
    ...
    def add_stage(
        ...
        self.stages.append({"type": stage_type, "config": stage_config, "condition": condition})

    def execute(
        self, model_path: str, output_path: str, pipeline_config: Optional[Dict[str, Any]] = None
    ) -> str:
        ...
        for stage in self.stages:
            ...
            if not converter.validate(current_model_path):
                raise ValueError(f"Model validation failed for stage: {stage['type']}")

            # Execute conversion
            stage_output_path = f"{output_path}.{stage['type']}"
            current_model_path = converter.convert(
                current_model_path, stage_output_path, stage["config"]
            )
