...
class TensorFlowConverter(BaseModelConverter):
    ...
    def convert(
        ...
        if not TENSORFLOW_AVAILABLE:
            raise ImportError(
                "TensorFlow is not available. Please install tensorflow to use this converter."
            )
...
class ONNXRuntimeConverter(BaseModelConverter):
    ...
    def convert(
        ...
        if not ONNX_AVAILABLE:
            raise ImportError(
                "ONNX is not available. Please install onnx and onnxruntime to use this converter."
            )
...
class SafetensorsConverter(BaseModelConverter):
    ...
    def get_metadata(self, model_path: str) -> Dict[str, Any]:
        ...
        return {"format": "safetensors", "metadata": metadata}
