...
class HardwareOptimizer:
    ...
    def optimize_for_gpu(self, model: nn.Module, gpu_config: Dict[str, Any]) -> nn.Module:
    ...
    def optimize_for_cpu(self, model: nn.Module, cpu_config: Dict[str, Any]) -> nn.Module:
    ...
    def optimize_for_mobile(self, model: nn.Module, mobile_config: Dict[str, Any]) -> nn.Module:
            ...
            for module in model.modules():
                if isinstance(module, torch.nn.Linear):
                    torch.nn.utils.prune.l1_unstructured(
                        module, name="weight", amount=mobile_config.get("pruning_amount", 0.3)
                    )

        return model

    def optimize_for_edge(self, model: nn.Module, edge_config: Dict[str, Any]) -> nn.Module:
        ...
        if edge_config.get("enable_pruning", True):
            for module in model.modules():
                if isinstance(module, torch.nn.Linear):
                    torch.nn.utils.prune.l1_unstructured(
                        module, name="weight", amount=edge_config.get("pruning_amount", 0.3)
                    )
...
class HardwareOptimizedConverter(BaseModelConverter):
    ...
    def convert(
        ...
        if config.get("target_hardware") == "gpu":
            model = self.optimizer.optimize_for_gpu(model, config.get("gpu_config", {}))
        elif config.get("target_hardware") == "cpu":
            model = self.optimizer.optimize_for_cpu(model, config.get("cpu_config", {}))
        elif config.get("target_hardware") == "mobile":
            model = self.optimizer.optimize_for_mobile(model, config.get("mobile_config", {}))
        elif config.get("target_hardware") == "edge":
            model = self.optimizer.optimize_for_edge(model, config.get("edge_config", {}))
    ...
    def get_metadata(self, model_path: str) -> Dict[str, Any]:
        ...
        return {
            ...
            / (1024 * 1024),
            "is_quantized": any(
                isinstance(m, torch.quantized.QDynamicLinear) for m in model.modules()
            ),
            "is_pruned": any(hasattr(m, "mask") for m in model.modules()),
        }
