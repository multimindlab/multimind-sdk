...
class AdvancedDistillation:
    ...
    def layer_distillation(
        self, student_model: nn.Module, teacher_model: nn.Module, layer_config: Dict[str, Any]
    ) -> nn.Module:
    ...
    def progressive_distillation(
        self, student_model: nn.Module, teacher_model: nn.Module, progressive_config: Dict[str, Any]
    ) -> nn.Module:
...
class DistillationConverter(BaseModelConverter):
    ...
    def get_metadata(self, model_path: str) -> Dict[str, Any]:
        ...
        model = torch.load(model_path)
        return {
            "distillation_methods": [method for method in dir(model) if method.endswith("_loss")],
