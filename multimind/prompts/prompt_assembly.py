...
from .advanced_prompting import AdvancedPrompting
...
class PromptAssembly:
    ...
    def __init__(
        self, llm: Optional[BaseLLM] = None, config: Optional[PromptAssemblyConfig] = None, **kwargs
    ):
        """
    ...
    async def assemble_prompt(
        ...
        **kwargs,
        ...
        # Generate prompt
        prompt = template.format(context=context, query=query, **kwargs)
    ...
    async def _process_documents(
        self, documents: List[Dict[str, Any]], max_docs: int, include_metadata: bool
    ) -> List[Dict[str, Any]]:
    ...
    async def _truncate_context(self, context: str, query: str, max_length: int) -> str:
    ...
    async def generate_custom_template(
        self, query: str, documents: List[Dict[str, Any]], template_style: str, **kwargs
    ) -> str:
    ...
    async def optimize_template(
        self, template: str, query: str, documents: List[Dict[str, Any]], **kwargs
    ) -> str:
    ...
    async def analyze_template_effectiveness(
        ...
        if not self.llm:
            return {"effectiveness": 0.0, "analysis": "LLM required for analysis"}
        ...
        try:
            score = float(
                re.search(r"Overall effectiveness score \(0-1\): ([\d.]+)", analysis).group(1)
            )
        except (AttributeError, ValueError):
            score = 0.0

        return {"effectiveness": score, "analysis": analysis}
