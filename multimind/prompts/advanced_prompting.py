...
class AdvancedPrompting:
    ...
    async def generate_prompt(
        ...
        **kwargs,
        ...
        if strategy == PromptStrategy.DIRECT:
            prompt = await self._generate_direct_prompt(template, context, **kwargs)
        elif strategy == PromptStrategy.STEP_BY_STEP:
            prompt = await self._generate_step_by_step_prompt(template, context, **kwargs)
        elif strategy == PromptStrategy.CHAIN_OF_THOUGHT:
            prompt = await self._generate_chain_of_thought_prompt(template, context, **kwargs)
        elif strategy == PromptStrategy.SELF_CONSISTENCY:
            prompt = await self._generate_self_consistency_prompt(template, context, **kwargs)
        else:  # TREE_OF_THOUGHT
            prompt = await self._generate_tree_of_thought_prompt(template, context, **kwargs)
        ...
        return GeneratedPrompt(
            ...
            context=context,
            metadata=kwargs,
            reasoning=await self._generate_reasoning(prompt, context, **kwargs),
        )

    async def optimize_context(self, context: PromptContext, **kwargs) -> PromptContext:
        """
        ...
        optimized_docs = await self._optimize_documents(
            context.retrieved_documents, context.query, **kwargs
        )

        # Optimize conversation history
        optimized_history = await self._optimize_history(
            context.conversation_history, context.query, **kwargs
        )

        # Update system state
        optimized_state = await self._optimize_state(context.system_state, context.query, **kwargs)
    ...
    async def refine_prompt(
        self, prompt: GeneratedPrompt, feedback: Optional[Dict[str, Any]] = None, **kwargs
    ) -> GeneratedPrompt:
        """
        ...
        # Get refined prompt
        refined_prompt = await self.model.generate(prompt=refinement_prompt, **kwargs)
        ...
        return GeneratedPrompt(
            ...
            reasoning=await self._generate_reasoning(refined_prompt, prompt.context, **kwargs),
        )

    def _get_template(self, prompt_type: PromptType) -> PromptTemplate:
    ...
    def _create_default_template(self, prompt_type: PromptType) -> PromptTemplate:
        ...
        return PromptTemplate(
            ...
            constraints={"max_tokens": 2000, "temperature": 0.7, "top_p": 0.9},
        )

    async def _generate_direct_prompt(
        self, template: PromptTemplate, context: PromptContext, **kwargs
    ) -> str:
    ...
    async def _generate_self_consistency_prompt(
        ...
        perspectives = await self.model.generate(prompt=perspectives_prompt, **kwargs)
    ...
    async def _optimize_documents(
        self, documents: List[Dict[str, Any]], query: str, **kwargs
    ) -> List[Dict[str, Any]]:
        """Optimize retrieved documents."""
        ...
        instructions = await self.model.generate(prompt=optimization_prompt, **kwargs)
        ...
        for doc in documents:
            ...
            relevance = await self.model.generate(prompt=relevance_prompt, **kwargs)
            ...
            if "relevant" in relevance.lower():
                ...
                optimized_content = await self.model.generate(prompt=content_prompt, **kwargs)

                optimized_docs.append({**doc, "content": optimized_content, "optimized": True})

        return optimized_docs

    async def _optimize_history(
        self, history: List[Dict[str, Any]], query: str, **kwargs
    ) -> List[Dict[str, Any]]:
        """Optimize conversation history."""
        # Generate optimization prompt
        optimization_prompt = f"""
        ...
        for turn in history:
            ...
            if "relevant" in relevance.lower():
                ...
                optimized_history.append({**turn, "content": optimized_content, "optimized": True})

        return optimized_history

    async def _optimize_state(self, state: Dict[str, Any], query: str, **kwargs) -> Dict[str, Any]:
        ...
        for key, value in state.items():
            ...
            if "relevant" in relevance.lower():
                ...
                optimized_value = await self.model.generate(prompt=value_prompt, **kwargs)

                optimized_state[key] = {"value": optimized_value, "optimized": True}

        return optimized_state

    async def _generate_reasoning(self, prompt: str, context: PromptContext, **kwargs) -> str:
