...
class EmbeddingModel:
    ...
    def __init__(
        ...
        if model_type == EmbeddingType.OPENAI:
            ...
            self.model = AutoModel.from_pretrained("hkunlp/instructor-xl", trust_remote_code=True)
            self.device = "cuda" if torch.cuda.is_available() else "cpu"
            self.model.to(self.device)
    ...
    async def generate_multi_vector_embedding(
        ...
        title_embedding = await self.generate_embedding(document["title"], config)

        content_embedding = await self.generate_embedding(document["content"], config)

        summary_embedding = None
        if "summary" in document:
            summary_embedding = await self.generate_embedding(document["summary"], config)

        metadata_embedding = None
        if "metadata" in document:
            metadata_text = json.dumps(document["metadata"])
            metadata_embedding = await self.generate_embedding(metadata_text, config)
    ...
    async def generate_batch_embeddings(
        ...
        for i in range(0, len(texts), config.batch_size):
            ...
            if self.model_type == EmbeddingType.OPENAI:
                batch_embeddings = await self._generate_openai_batch_embeddings(batch, config)

            elif self.model_type == EmbeddingType.COHERE:
                batch_embeddings = await self._generate_cohere_batch_embeddings(batch, config)

            elif self.model_type == EmbeddingType.HUGGINGFACE:
                batch_embeddings = await self._generate_huggingface_batch_embeddings(batch, config)
                ...
                batch_embeddings = await self._generate_instructor_batch_embeddings(batch, config)
    ...
    async def _generate_openai_embedding(self, text: str, config: EmbeddingConfig) -> List[float]:
        """Generate embedding using OpenAI."""
        # Use new OpenAI client API (v1.0+)
        if hasattr(self, "openai_client") and self.openai_client is not None:
            response = await self.openai_client.embeddings.create(
                input=text, model=self.model_name, **config.custom_params
            )
    ...
    async def _generate_cohere_embedding(self, text: str, config: EmbeddingConfig) -> List[float]:
        """Generate embedding using Cohere."""
        response = self.model.embed(texts=[text], model=self.model_name, **config.custom_params)
    ...
    async def _generate_huggingface_embedding(
        self, text: str, config: EmbeddingConfig
    ) -> List[float]:
        """Generate embedding using HuggingFace model."""
        # Tokenize
        inputs = self.tokenizer(
            text, max_length=config.max_length, padding=True, truncation=True, return_tensors="pt"
        ).to(self.device)
    ...
    async def _generate_openai_batch_embeddings(
        ...
        if hasattr(self, "openai_client") and self.openai_client is not None:
            response = await self.openai_client.embeddings.create(
                input=texts, model=self.model_name, **config.custom_params
            )
    ...
    async def _generate_cohere_batch_embeddings(
        ...
        response = self.model.embed(texts=texts, model=self.model_name, **config.custom_params)
    ...
    async def _generate_huggingface_batch_embeddings(
        ...
        # Tokenize
        inputs = self.tokenizer(
            texts, max_length=config.max_length, padding=True, truncation=True, return_tensors="pt"
        ).to(self.device)
        ...
        if config.normalize:
            embeddings_list = [
                self._normalize_embedding(embedding) for embedding in embeddings_list
            ]
...
async def get_embedding(
    text: str, model_name: str = "sentence-transformers/all-MiniLM-L6-v2"
) -> List[float]:
