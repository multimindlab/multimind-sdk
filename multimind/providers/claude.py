...
class ClaudeProvider(ProviderAdapter):
    ...
    async def generate_text(self, model: str, prompt: str, **kwargs) -> GenerationResult:
        ...
        try:
            ...
            logger.error(
                "The Claude API client is missing the 'messages.create' method. Please update the client."
            )
    ...
    async def chat(
        self, messages: List[Dict[str, str]], model: str = "claude-3-sonnet", **kwargs
    ) -> GenerationResult:
    ...
    async def generate_embeddings(
        self, text: str, model: str = "claude-3-sonnet", **kwargs
    ) -> EmbeddingResult:
        """Generate embeddings using Claude's API."""
        raise NotImplementedError("Claude API does not provide embeddings.")

    async def analyze_image(
        self, image_data: bytes, prompt: str, model: str = "claude-3-sonnet", **kwargs
    ) -> ImageAnalysisResult:
    ...
    async def estimate_cost(
        ...
        pricing = self.metadata.pricing.get(model, {"input": 0.0, "output": 0.0})
        return (pricing["input"] * input_tokens + pricing["output"] * (output_tokens or 0)) / 1000
    ...
    async def estimate_latency(
        ...
        latency = (
            self.metadata.latency.get(model, {"p50": 0, "p95": 0})
