...
class EnhancedDocumentProcessor:
    ...
    async def process_document(
        ...
        if self.chunking_strategy == ChunkingStrategy.SEMANTIC:
            chunks = await self.semantic_chunker.chunk_document(
                text, metadata=extracted_metadata, **kwargs
            )
        else:
            # Implement other chunking strategies
            raise NotImplementedError(f"Chunking strategy {self.chunking_strategy} not implemented")
        ...
        if self.config.generate_embeddings:
            for chunk in chunks:
                ...
                embedding = (
                    embeddings_result[0]
                    if isinstance(embeddings_result, list) and len(embeddings_result) > 0
    ...
    async def process_documents(
        self, documents: List[str], metadata_list: Optional[List[Dict[str, Any]]] = None, **kwargs
    ...
    async def merge_chunks(
        self, chunks: List[DocumentChunk], max_tokens: Optional[int] = None, **kwargs
    ) -> List[DocumentChunk]:
        """
    ...
    def _should_merge_chunks(
        self, chunk1: DocumentChunk, chunk2: DocumentChunk, max_tokens: Optional[int]
    ) -> bool:
        ...
        similarity = self.semantic_chunker._cosine_similarity(chunk1.embedding, chunk2.embedding)

        # Check token count if max_tokens is specified
        if max_tokens:
            combined_tokens = len(
                self.semantic_chunker.tokenizer.encode(chunk1.text + " " + chunk2.text)
            )
    ...
    def _merge_two_chunks(self, chunk1: DocumentChunk, chunk2: DocumentChunk) -> DocumentChunk:
        ...
        return DocumentChunk(
            ...
            embedding=(
                np.mean([chunk1.embedding, chunk2.embedding], axis=0)
