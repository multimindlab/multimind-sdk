        ...
        if TRANSFORMERS_AVAILABLE:
            ...
            if _AUTO_MODEL_CLASS is not None:
                ...
                try:
                    ...
                    raise ImportError(
                        "Unable to load BART model. Please ensure transformers is properly installed."
                    )
        else:
            self.tokenizer = None
...
class CharacterChunker:
    ...
    def chunk(self, text: str) -> List[str]:
        return [
            text[i : i + self.window_size]
            for i in range(0, len(text), self.stride)
            if text[i : i + self.window_size]
        ]
...
class HTMLXMLChunker:
    ...
    def chunk(self, html: str) -> List[str]:
        try:
            ...
            raise ImportError(
                "BeautifulSoup4 is required for HTML/XML chunking. Install with: pip install beautifulsoup4"
            )
        soup = BeautifulSoup(html, "html.parser")
        if self.tag:
            return [str(e) for e in soup.find_all(self.tag)]
        return (
            [str(e) for e in soup.body.find_all(recursive=False)]
            if soup.body
            else [str(e) for e in soup.find_all(recursive=False)]
        )
...
class CustomChunker:
    ...
    def __init__(
        self, chunk_fn: Optional[Callable[[str], List[str]]] = None, regex: Optional[str] = None
    ):
...
class MetadataExtractor:
    ...
    def extract_metadata(self, text: str) -> Dict[str, Any]:
        ...
        # Extract entities
        entities = {
            ent.label_: [e.text for e in doc.ents if e.label_ == ent.label_] for ent in doc.ents
        }
        ...
        return {"entities": entities, "key_phrases": key_phrases, "statistics": stats}
...
class SpreadsheetChunker:
    ...
    def chunk(self, file_path: str) -> list:
        try:
            ...
            raise ImportError(
                "pandas is required for SpreadsheetChunker. Install with: pip install pandas openpyxl"
            )
...
class PresentationChunker:
    ...
    def chunk(self, file_path: str) -> list:
        try:
            ...
            raise ImportError(
                "python-pptx is required for PresentationChunker. Install with: pip install python-pptx"
            )
