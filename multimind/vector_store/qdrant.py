...
class QdrantBackend(VectorStoreBackend):
    ...
    async def add_vectors(self, vectors, metadatas, documents, ids=None):
        ...
        for i in range(n):
            ...
            points.append(
                PointStruct(
                    id=ids[i], vector=np.array(vectors[i], dtype=np.float32), payload=payload
                )
            )
        loop = asyncio.get_event_loop()
        await loop.run_in_executor(
            None, lambda: self._client.upsert(collection_name=self.collection, points=points)
        )
    ...
    async def search(
        ...
        if filter_criteria:
            conditions = [
                FieldCondition(key=k, match=MatchValue(value=v)) for k, v in filter_criteria.items()
            ]
    ...
    async def delete_vectors(self, ids):
        loop = asyncio.get_event_loop()
        await loop.run_in_executor(
            None, lambda: self._client.delete(collection_name=self.collection, points=ids)
        )
    ...
    async def clear(self):
        ...
        await loop.run_in_executor(
            None,
            lambda: self._client.delete(collection_name=self.collection, filter=Filter(must=[])),
        )
