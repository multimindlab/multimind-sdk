...
class MilvusBackend(VectorStoreBackend):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        fields = [
            FieldSchema(
                name="id", dtype=DataType.VARCHAR, is_primary=True, auto_id=False, max_length=64
            ),
    ...
    async def add_vectors(
        ...
        ids=None,
        partition_name: Optional[str] = None,
        custom_fields_data: Optional[List[Dict[str, Any]]] = None,
    ):
        n = len(vectors)
        ...
        if self.custom_fields:
            for field in self.custom_fields:
                fname = field["name"]
                values = [
                    d.get(fname) if d else None
                    for d in (custom_fields_data or [{} for _ in range(n)])
                ]
                data.append(values)
    ...
    async def upsert_vectors(
        ...
        await self.add_vectors(
            vectors, metadatas, documents, ids, partition_name, custom_fields_data
        )
    ...
    async def search(
        ...
        partition_name: Optional[str] = None,
        hybrid_filter: Optional[str] = None,
    ) -> List[SearchResult]:
        loop = asyncio.get_event_loop()
    ...
    async def create_index(self, field_name: str, index_params: Dict[str, Any]):
        ...
        await loop.run_in_executor(
            None,
            lambda: self.collection.create_index(field_name=field_name, index_params=index_params),
        )
