...
class MongoDBAtlasBackend(VectorStoreBackend):
    ...
    async def search(
        ...
        pipeline = [
            {
                "$search": {
                    "index": "default",
                    "knnBeta": {"vector": query_vector, "path": self.vector_field, "k": k},
                }
            }
        ]
        if filter_criteria:
        ...
        if metadata_fields:
            ...
            projection.update(
                {"_id": 1, "score": {"$meta": "searchScore"}, "document": 1, "metadata": 1}
            )
        ...
        results = await loop.run_in_executor(
            None, lambda: list(self._collection.aggregate(pipeline))
        )
        search_results = [
            SearchResult(
                id=doc.get("_id"),
    ...
    async def delete_vectors(self, ids):
        ...
        await loop.run_in_executor(
            None, lambda: self._collection.delete_many({"_id": {"$in": ids}})
        )
