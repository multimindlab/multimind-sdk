        ...
        def _search():
            with tiledb.DenseArray(self.array_uri, mode="r") as A:
                ...
                for i, vec in enumerate(vectors):
                    dist = np.linalg.norm(
                        np.array(vec, dtype=np.float32) - np.array(query_vector, dtype=np.float32)
                    )
                ...
                for idx, dist in scores[:k]:
                    results.append(
                        SearchResult(
                            ...
                            score=-dist,
                            metadata=(
                                metadatas[idx].decode()
                                if hasattr(metadatas[idx], "decode")
                                else metadatas[idx]
                            ),
                            document=(
                                documents[idx].decode()
                                if hasattr(documents[idx], "decode")
                                else documents[idx]
                            ),
                        )
                    )
