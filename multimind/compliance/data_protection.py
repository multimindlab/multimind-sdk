...
class DataProtectionManager(BaseModel):
    ...
    async def protect_data(
        self, data: Any, category: DataCategory, metadata: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
    ...
    async def unprotect_data(self, protected_data: Dict[str, Any]) -> Any:
        """Unprotect data."""
        # Verify integrity
    ...
    def _generate_pseudonym(self, value: Union[str, int, float]) -> str:
        ...
        return hmac.new(self.pseudonymization_salt, value_str, hashlib.sha256).hexdigest()
    ...
    async def get_protection_status(self, data: Dict[str, Any]) -> Dict[str, Any]:
