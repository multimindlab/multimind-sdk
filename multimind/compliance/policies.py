...
class CompliancePolicyEngine(BaseModel):
    ...
    async def register_handler(self, rule_id: str, handler: Callable) -> None:
        """Register a handler for a rule."""
        self.rule_handlers[rule_id] = handler

    async def evaluate_policy(self, context: Dict[str, Any]) -> List[PolicyViolation]:
        ...
        for rule in sorted_rules:
            ...
            if not self._evaluate_conditions(rule, context):
                ...
                violation = PolicyViolation(
                    ...
                    details={"context": context, "rule": rule.dict()},
    ...
    async def resolve_violation(
        self, violation_id: str, resolution: str
    ...
    async def get_active_violations(
        self, rule_id: Optional[str] = None, severity: Optional[str] = None
    ...
    async def _execute_actions(
        self, rule: PolicyRule, context: Dict[str, Any], violation: PolicyViolation
    ) -> None:
        """Execute rule actions."""
        for action in rule.actions:
            ...
            if action_type == "log":
                ...
                message = action_params.get(
                    "message", f"Policy violation: {violation.violation_id}"
                )
                logger.info("[COMPLIANCE NOTIFY] To: %s | Message: %s", recipient, message)
            elif action_type == "block":
                # Block operation by raising an exception
                raise Exception(
                    f"Operation blocked due to policy violation: {violation.violation_id} (Rule: {rule.name})"
                )
