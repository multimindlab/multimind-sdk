...
class ComplianceVisualizer:
    ...
    def plot_metrics_history(
        self, metrics: List[str] = None, save_path: Optional[str] = None
    ) -> go.Figure:
        ...
        fig = make_subplots(
            rows=len(metrics), cols=1, subplot_titles=[f"{m.capitalize()} Score" for m in metrics]
        )

        for i, metric in enumerate(metrics, 1):
            fig.add_trace(
                go.Scatter(y=df[f"{metric}_score"], name=metric.capitalize(), mode="lines+markers"),
                row=i,
                col=1,
            )

        fig.update_layout(
            height=300 * len(metrics), title_text="Compliance Metrics History", showlegend=True
        )
    ...
    def plot_violations_heatmap(self, save_path: Optional[str] = None) -> go.Figure:
        ...
        pivot = df.pivot_table(values="value", index="hour", columns="metric", aggfunc="mean")

        fig = go.Figure(
            data=go.Heatmap(z=pivot.values, x=pivot.columns, y=pivot.index, colorscale="RdYlGn_r")
        )

        fig.update_layout(
            title="Compliance Violations Heatmap", xaxis_title="Metric", yaxis_title="Hour of Day"
        )
    ...
    def create_dashboard(self, port: int = 8050, debug: bool = False) -> None:
        ...
        app.layout = html.Div(
            [
                ...
                dcc.Tabs(
                    [
                        dcc.Tab(
                            label="Metrics History", children=[dcc.Graph(id="metrics-history")]
                        ),
                        dcc.Tab(
                            label="Violations Heatmap",
                            children=[dcc.Graph(id="violations-heatmap")],
                        ),
                        dcc.Tab(label="Recommendations", children=[html.Div(id="recommendations")]),
                    ]
                ),
                dcc.Interval(
    ...
    def plot_compliance_radar(
        ...
        fig.add_trace(
            go.Scatterpolar(r=values, theta=categories, fill="toself", name="Compliance Scores")
        )
    ...
    def plot_violation_timeline(self, save_path: Optional[str] = None) -> go.Figure:
        ...
        fig = px.scatter(
            df, x="timestamp", y="score", color="metric", title="Compliance Violations Timeline"
        )
