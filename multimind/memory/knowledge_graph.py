...
class KnowledgeGraphMemory(BaseMemory):
    ...
    async def save(self) -> None:
        ...
        if self.storage_path:
            ...
            with open(self.storage_path, "w") as f:
                json.dump({"messages": self.messages, "graph": nx.node_link_data(self.graph)}, f)
    ...
    def _update_graph(
        self, entities: Set[Tuple[str, str]], relationships: List[Tuple[str, str, str, str]]
    ) -> None:
    ...
    def _trim_graph(self) -> None:
        ...
        nodes_by_time = sorted(
            ...
            key=lambda x: (
                min(edge["timestamp"] for edge in self.graph.edges(x[0], data=True))
                if self.graph.edges(x[0])
    ...
    def get_entity_context(self, entity: str) -> str:
        ...
        for rel in relationships:
            context.append(f"{rel['source']} {rel['relationship']} ({rel['type']}) {rel['target']}")
            ...
            entities.append(
                {
                    ...
                    "score": (
                        degree_centrality[node] + betweenness_centrality[node] + pagerank[node]
                    )
                    / 3,
                }
            )
