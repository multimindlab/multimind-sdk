...
class CountMinSketch:
    ...
    def estimate(self, key: str) -> int:
        ...
        return min(
            self.counts[i, mmh3.hash(key, self.seeds[i]) % self.width] for i in range(self.depth)
        )
...
class BloomFilter:
    ...
    def contains(self, key: str) -> bool:
        ...
        return all(
            self.bits[mmh3.hash(key, self.seeds[i]) % self.size] for i in range(self.num_hashes)
        )
...
class SketchMemory(BaseMemory):
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "false_positive_rate": (
                self.false_positives / self.total_queries if self.total_queries > 0 else 0.0
            ),
            "avg_access_count": (
                np.mean([len(times) for times in self.access_times.values()])
