...
class MetaMemory(BaseMemory):
    ...
    async def get(self, key: str) -> Any:
        ...
        self.usage_history[key].append(
            {"timestamp": datetime.now(), "type": "read", "success": value is not None}
        )
    ...
    async def get_important_memories(self, threshold: float = 0.5) -> List[str]:
        ...
        return [key for key, score in self.importance_scores.items() if score >= threshold]

    async def get_frequently_accessed(self, min_accesses: int = 5) -> List[str]:
        """Get keys of frequently accessed memories."""
        return [key for key, count in self.access_counts.items() if count >= min_accesses]

    async def get_successful_memories(self, threshold: float = 0.7) -> List[str]:
        """Get keys of memories with high success rates."""
        return [key for key, rate in self.success_rates.items() if rate >= threshold]
    ...
    async def get_memory_stats(self, key: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_accessed": (
                self.usage_history[key][-1]["timestamp"]
                if key in self.usage_history and self.usage_history[key]
                else None
            ),
        }
    ...
    async def cleanup(self, min_importance: float = 0.1) -> None:
        ...
        keys_to_remove = [
            key for key, score in self.importance_scores.items() if score < min_importance
        ]
