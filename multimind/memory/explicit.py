...
class ExplicitMemory(BaseMemory):
    ...
    def __init__(self, recall_threshold: float = 0.7, max_facts: int = 10000, **kwargs):
    ...
    async def get_related_facts(
        self, fact_id: str, include_metadata: bool = True
    ) -> List[Dict[str, Any]]:
    ...
    async def record_recall(
        self, fact_id: str, recall_score: float, context: Optional[Dict[str, Any]] = None
    ) -> None:
    ...
    async def get_fact_stats(
        self, fact_id: str, time_window: Optional[timedelta] = None
    ) -> Dict[str, Any]:
    ...
    async def update_fact(self, fact_id: str, updates: Dict[str, Any]) -> None:
