...
class SimpleMemory(BaseMemory):
    ...
    async def get_messages_by_role(self, role: str) -> List[Dict[str, Any]]:
        ...
        return [m["message"] for m in self.messages if m["message"].get("role") == role]

    async def get_messages_in_timeframe(
        self, start_time: datetime, end_time: datetime
    ) -> List[Dict[str, Any]]:
        """Get messages within a timeframe."""
        return [m["message"] for m in self.messages if start_time <= m["timestamp"] <= end_time]
    ...
    async def get_messages_with_metadata(
        ...
        return [
            m["message"] for m in self.messages if m["metadata"].get(metadata_key) == metadata_value
        ]
    ...
    async def get_message_with_metadata(self, index: int) -> Optional[Dict[str, Any]]:
