...
class ProspectiveMemory(BaseMemory):
            ...
            if intention["status"] == "pending":
                if (min_priority is None or intention["priority"] >= min_priority) and (
                    ...
                    if include_dependencies:
                        intention["dependencies"] = list(
                            self.intention_graph.predecessors(intention_id)
                        )
            ...
            if now <= reminder_time <= end_time:
                ...
                for reminder in self.reminders[intention_id]:
                    if reminder["time"] == reminder_time and reminder["status"] == "pending":
                        reminders.append({"intention": intention, "reminder": reminder})
        return reminders

    async def get_dependent_intentions(
        self, intention_id: str, max_depth: int = 2
    ) -> List[Dict[str, Any]]:
    ...
    async def update_intention(self, intention_id: str, updates: Dict[str, Any]) -> None:
        ...
        if intention_id in self.intentions:
            ...
            if "description" in updates:
                await self.semantic_memory.add(
                    intention_id, updates["description"], intention["metadata"]
                )
            if "planned_time" in updates:
                await self.temporal_memory.add(
                    intention_id, updates["planned_time"], intention["metadata"]
                )

            # Update graph
            self.intention_graph.nodes[intention_id].update(updates)

    async def mark_reminder_complete(self, intention_id: str, reminder_time: datetime) -> None:
    ...
    async def mark_intention_complete(
        self, intention_id: str, completion_time: Optional[datetime] = None
    ) -> None:
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "pending_intentions": len(
                [i for i in self.intentions.values() if i["status"] == "pending"]
            ),
            "completed_intentions": len(
                [i for i in self.intentions.values() if i["status"] == "complete"]
            ),
