...
class ContextualMemory(BaseMemory):
    ...
    async def add_message(self, message: Dict[str, str]) -> None:
        ...
        if self.enable_summarization:
            ...
            if (
                current_time - self.last_summarization
            ).total_seconds() > self.summarization_interval:
    ...
    async def _find_similar_contexts(self, context: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for i, existing_embedding in enumerate(self.context_embeddings):
            ...
            if similarity >= self.context_similarity_threshold:
                similarities.append(
                    {"context_id": self.contexts[i]["id"], "similarity": similarity}
                )

        return sorted(similarities, key=lambda x: x["similarity"], reverse=True)

    async def _create_context(
        self, context: Dict[str, Any], similar_contexts: Optional[List[Dict[str, Any]]] = None
    ) -> None:
    ...
    async def _merge_contexts(self, target_id: str, source_context: Dict[str, Any]) -> None:
        """Merge source context into target context."""
        target_idx = next(i for i, ctx in enumerate(self.contexts) if ctx["id"] == target_id)
        ...
        # Update embedding
        context_text = " ".join(msg["content"] for msg in self.contexts[target_idx]["messages"])
    ...
    async def _add_relationship(
        self, context_id1: str, context_id2: str, relationship_type: Optional[str] = None
    ) -> None:
    ...
    async def _maintain_context_window(self) -> None:
        ...
        if len(self.contexts) > self.max_contexts:
            ...
            sorted_contexts = sorted(self.contexts, key=lambda x: self.context_weights[x["id"]])
    ...
    async def _remove_context(self, context_id: str) -> None:
        ...
        context_idx = next(i for i, ctx in enumerate(self.contexts) if ctx["id"] == context_id)
        ...
        for other_id in self.relationships:
            self.relationships[other_id] = {
                rel for rel in self.relationships[other_id] if not rel.startswith(f"{context_id}:")
            }
    ...
    async def load(self) -> None:
        ...
        if self.storage_path and self.storage_path.exists():
            with open(self.storage_path) as f:
                ...
                self.context_metadata = {
                    k: {**v, "entities": set(v["entities"]), "keywords": set(v["keywords"])}
                ...
                for context in self.contexts:
                    context_text = " ".join(msg["content"] for msg in context["messages"])
                    self.context_embeddings.append(await self.llm.embeddings(context_text))
    ...
    async def get_context_chain(self, context_id: str, max_depth: int = 3) -> List[Dict[str, Any]]:
        ...
        async def traverse(current_id: str, depth: int) -> None:
            ...
            current_context = next(ctx for ctx in self.contexts if ctx["id"] == current_id)
    ...
    async def get_context_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "relationship_types": {rel_type: 0 for rel_type in self.relationship_types},
            "topic_distribution": {},
            "sentiment_distribution": {},
            "entity_frequency": {},
            "keyword_frequency": {},
            "weight_distribution": {"high": 0, "medium": 0, "low": 0},  # > 0.7  # 0.3-0.7  # < 0.3
            "evolution_stages": {},
            "summarization_stats": {
        ...
        for context in self.contexts:
            ...
            for entity in self.context_metadata[context["id"]]["entities"]:
                stats["entity_frequency"][entity] = stats["entity_frequency"].get(entity, 0) + 1
            ...
            if stage:
                stats["evolution_stages"][stage] = stats["evolution_stages"].get(stage, 0) + 1
    ...
    async def get_context_suggestions(self) -> List[Dict[str, Any]]:
        ...
        if len(stats["topic_distribution"]) < 3:
            suggestions.append(
                {"type": "topic_diversity", "suggestion": "Consider adding more diverse topics"}
            )

        # Check summarization status
        if stats["summarization_stats"]["unsummarized"] > len(self.contexts) * 0.3:
            suggestions.append(
                {"type": "summarization", "suggestion": "Consider running context summarization"}
            )
