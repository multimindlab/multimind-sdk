...
class TemporalMemory(BaseMemory):
    ...
    async def _determine_relationship_type(
        self, event1: Dict[str, Any], event2: Dict[str, Any], similarity: float
    ) -> Optional[str]:
    ...
    async def _update_learning_progress(self, event_id: str) -> None:
        ...
        relationship_count = sum(
            len(relationships) for relationships in self.relationships[event_id].values()
        )
        ...
        event["metadata"]["learning_progress"] = min(
            1.0, event["metadata"]["learning_progress"] + progress
        )
    ...
    async def _remove_event(self, event_id: str) -> None:
        ...
        # Remove from history
        if self.enable_history:
            self.event_history = [e for e in self.event_history if e["event_id"] != event_id]
    ...
    async def load(self) -> None:
        ...
        if self.storage_path and self.storage_path.exists():
            with open(self.storage_path) as f:
                ...
                self.event_embeddings = []
                for event in self.events:
                    self.event_embeddings.append(self.llm.embeddings(event["content"]))
    ...
    async def get_temporal_memory_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "temporal_type_distribution": {
                event_type: sum(
                    1 for e in self.events if e["metadata"]["temporal_type"] == event_type
                )
                for event_type in set(
                    e["metadata"]["temporal_type"]
                    for e in self.events
                    if e["metadata"]["temporal_type"]
                )
            },
            "relationship_stats": {
            ...
            "learning_stats": {
                "average_progress": (
                    sum(e["metadata"]["learning_progress"] for e in self.events) / len(self.events)
                    if self.events
                    else 0
                ),
                "events_with_progress": sum(
                    1 for e in self.events if e["metadata"]["learning_progress"] > 0
                ),
            },
            "evolution_stats": {
                "stage_distribution": {
                ...
                "average_stage": (
                    sum(e["metadata"]["evolution_stage"] for e in self.events) / len(self.events)
                    if self.events
                    else 0
                ),
            },
            "validation_stats": {
                "average_score": (
                    sum(e["metadata"]["validation_score"] for e in self.events) / len(self.events)
                    if self.events
                    else 0
                ),
                "validated_events": sum(
                    1 for e in self.events if e["metadata"]["validation_score"] >= 0.8
                ),
            },
        }
