...
class AdaptiveMemory(BaseMemory):
    ...
    async def add_memory(
        ...
        self.context_sizes[memory_id] = initial_context_size or self.min_context_size
    ...
    async def get_memory(
        self, memory_id: str, query: Optional[str] = None
    ...
    async def _adjust_context_size(self, memory_id: str, confidence: float) -> None:
        ...
        if confidence < self.confidence_threshold:
            ...
            new_size = min(current_size * 2, self.max_context_size)
        else:
            # Decrease context size
            new_size = max(current_size // 2, self.min_context_size)
