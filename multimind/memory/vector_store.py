...
class VectorStoreMemory(BaseMemory):
    ...
    async def add(
        ...
        # Prepare metadata
        memory_metadata = (
            {"content": content, "timestamp": datetime.now().isoformat(), **(metadata or {})}
        ...
        if (
            self.enable_pruning
            and (datetime.now() - self.last_pruning).total_seconds() >= self.pruning_interval
        ):
    ...
    async def get(self, memory_id: str, update_access: bool = True) -> Optional[Dict[str, Any]]:
        """Get a vector by ID."""
    ...
    async def search(
        self, query: str, k: int = 5, filter_func: Optional[callable] = None
    ) -> List[Dict[str, Any]]:
    ...
    async def _prune_vectors(self) -> None:
        ...
        for result in results:
            ...
            ).total_seconds() > self.pruning_interval or access_count < self.pruning_threshold:
