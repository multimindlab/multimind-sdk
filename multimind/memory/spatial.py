...
class SpatialMemory(BaseMemory):
    ...
    async def add_message(self, message: Dict[str, str]) -> None:
        ...
        self.relationships[location_id] = {rel_type: [] for rel_type in self.relationship_types}
    ...
    async def _determine_relationship_type(
        self, location1: Dict[str, Any], location2: Dict[str, Any], similarity: float
    ) -> Optional[str]:
    ...
    async def _update_learning_progress(self, location_id: str) -> None:
        ...
        relationship_count = sum(
            len(relationships) for relationships in self.relationships[location_id].values()
        )
        ...
        location["metadata"]["learning_progress"] = min(
            1.0, location["metadata"]["learning_progress"] + progress
        )
    ...
    async def _remove_location(self, location_id: str) -> None:
        ...
        if self.enable_history:
            self.location_history = [
                l for l in self.location_history if l["location_id"] != location_id
            ]
    ...
    async def load(self) -> None:
        ...
        if self.storage_path and self.storage_path.exists():
            with open(self.storage_path) as f:
                ...
                self.location_embeddings = []
                for location in self.locations:
                    self.location_embeddings.append(self.llm.embeddings(location["content"]))
    ...
    async def get_spatial_memory_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "learning_stats": {
                "average_progress": (
                    sum(l["metadata"]["learning_progress"] for l in self.locations)
                    / len(self.locations)
                    if self.locations
                    else 0
                ),
                "locations_with_progress": sum(
                    1 for l in self.locations if l["metadata"]["learning_progress"] > 0
                ),
            },
            "evolution_stats": {
                "stage_distribution": {
                    stage: sum(
                        1 for l in self.locations if l["metadata"]["evolution_stage"] == stage
                    )
                    for stage in range(4)
                },
                "average_stage": (
                    sum(l["metadata"]["evolution_stage"] for l in self.locations)
                    / len(self.locations)
                    if self.locations
                    else 0
                ),
            ...
            "validation_stats": {
                "average_score": (
                    sum(l["metadata"]["validation_score"] for l in self.locations)
                    / len(self.locations)
                    if self.locations
                    else 0
                ),
                "validated_locations": sum(
                    1 for l in self.locations if l["metadata"]["validation_score"] >= 0.8
                ),
            },
        }
