...
class DeclarativeMemory(BaseMemory):
    ...
    async def add_message(self, message: Dict[str, str]) -> None:
        ...
        if self.enable_contradiction_detection:
            current_time = datetime.now()
            if (
                current_time - self.last_contradiction
            ).total_seconds() > self.contradiction_interval:
    ...
    async def _update_learning_progress(self, fact_id: str) -> None:
        ...
        fact["metadata"]["learning_progress"] = min(
            1.0, fact["metadata"]["learning_progress"] + progress
        )
    ...
    async def _remove_fact(self, fact_id: str) -> None:
        ...
        # Remove from history
        if self.enable_history:
            self.fact_history = [f for f in self.fact_history if f["fact_id"] != fact_id]
    ...
    async def load(self) -> None:
        ...
        if self.storage_path and self.storage_path.exists():
            with open(self.storage_path) as f:
                ...
                self.fact_embeddings = []
                for fact in self.facts:
                    self.fact_embeddings.append(await self.llm.embeddings(fact["content"]))
    ...
    async def get_declarative_memory_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "verification_stats": {
                "average_score": (
                    sum(f["metadata"]["verification_score"] for f in self.facts) / len(self.facts)
                    if self.facts
                    else 0
                ),
                "verified_facts": sum(
            ...
            "consistency_stats": {
                "average_score": (
                    sum(f["metadata"]["consistency_score"] for f in self.facts) / len(self.facts)
                    if self.facts
                    else 0
                ),
                "consistent_facts": sum(
                    1 for f in self.facts if f["metadata"]["consistency_score"] >= 0.8
                ),
            },
            "learning_stats": {
                "average_progress": (
                    sum(f["metadata"]["learning_progress"] for f in self.facts) / len(self.facts)
                    if self.facts
                    else 0
                ),
                "facts_with_progress": sum(
                    1 for f in self.facts if f["metadata"]["learning_progress"] > 0
                ),
            },
            "evolution_stats": {
                "stage_distribution": {
                ...
                "average_stage": (
                    sum(f["metadata"]["evolution_stage"] for f in self.facts) / len(self.facts)
                    if self.facts
                    else 0
                ),
            },
            "validation_stats": {
                "average_score": (
                    sum(f["metadata"]["validation_score"] for f in self.facts) / len(self.facts)
                    if self.facts
                    else 0
                ),
                "validated_facts": sum(
                    1 for f in self.facts if f["metadata"]["validation_score"] >= 0.8
                ),
            },
        }
        ...
        if self.enable_knowledge_integration:
            stats["integration_stats"] = {
                ...
                "average_score": (
                    sum(integration["score"] for integration in self.integrated_knowledge.values())
                    / len(self.integrated_knowledge)
        ...
        if self.enable_semantic_reasoning:
            stats["reasoning_stats"] = {
                ...
                "average_score": (
                    sum(reasoning["score"] for reasoning in self.semantic_reasoning.values())
                    / len(self.semantic_reasoning)
                    if self.semantic_reasoning
                    else 0
                ),
                "reasoning_types": {
                    reasoning["type"]: sum(
                        1
                        for r in self.semantic_reasoning.values()
        ...
        if self.enable_uncertainty:
            stats["uncertainty_stats"] = {
                "average_score": (
                    sum(measures["score"] for measures in self.uncertainty_measures.values())
                    / len(self.uncertainty_measures)
                    if self.uncertainty_measures
                    else 0
                ),
                "uncertainty_types": {
                    measures["type"]: sum(
                        1
                        for m in self.uncertainty_measures.values()
        ...
        if self.enable_contradiction_detection:
            stats["contradiction_stats"] = {
                "total_contradictions": sum(
                    len(contradictions) for contradictions in self.contradictions.values()
                ),
                "contradiction_types": {
                    contradiction["type"]: sum(
                        1 for c in contradictions if c["type"] == contradiction["type"]
                    )
        ...
        if self.enable_temporal_reasoning:
            stats["temporal_stats"] = {
                ...
                "average_score": (
                    sum(relation["score"] for relation in self.temporal_relations.values())
                    / len(self.temporal_relations)
                    if self.temporal_relations
                    else 0
                ),
                "temporal_types": {
                    relation["type"]: sum(
                        1 for r in self.temporal_relations.values() if r["type"] == relation["type"]
                    )
        ...
        if self.enable_causal_analysis:
            stats["causal_stats"] = {
                "total_chains": sum(
                    len(chains["chains"]) for chains in self.causal_chains.values()
                ),
                "average_score": (
                    sum(chains["score"] for chains in self.causal_chains.values())
                ...
                "chain_types": {
                    chain["chain_type"]: sum(
                        1 for c in chains["chains"] if c["chain_type"] == chain["chain_type"]
                    )
        ...
        if self.enable_knowledge_graph:
            stats["graph_stats"] = {
                ...
                "node_types": {
                    node["type"]: sum(
                        1 for n in self.knowledge_graph.values() if n["type"] == node["type"]
                    )
