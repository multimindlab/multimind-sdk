...
class BayesianMemory(BaseMemory):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.cluster_stats: Dict[int, Dict[str, Any]] = defaultdict(
            lambda: {"count": 0, "mean_embedding": None, "covariance": None, "weight": 0.0}
        )
    ...
    async def get_memories_by_cluster(
        self, cluster_id: int, include_stats: bool = False
    ) -> List[Dict[str, Any]]:
    ...
    async def get_cluster_stats(self, cluster_id: int) -> Dict[str, Any]:
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_cluster_size": (
                self.total_memories / self.active_clusters if self.active_clusters > 0 else 0
            ),
            "mixture_weights": (
                self.mixture_model.weights_ if hasattr(self.mixture_model, "weights_") else []
            ),
        }
