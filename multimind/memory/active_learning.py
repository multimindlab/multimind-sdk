...
class ActiveLearningMemory(BaseMemory):
    ...
    async def _maintain_item_limit(self) -> None:
        ...
        if len(self.feedback) > self.max_feedback:
            ...
            sorted_feedback = sorted(
                self.feedback, key=lambda x: datetime.fromisoformat(x["timestamp"])
            )
    ...
    async def get_active_learning_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "feedback_stats": {
                ...
                "average_feedback_per_item": (
                    len(self.feedback) / len(self.items) if self.items else 0
                ),
            },
            "reinforcement_stats": {
                ...
                "average_reinforcement_per_item": (
                    sum(len(reinforcement) for reinforcement in self.reinforcement.values())
                    / len(self.reinforcement)
    ...
    async def get_active_learning_suggestions(self) -> List[Dict[str, Any]]:
        ...
        if stats["feedback_stats"]["average_feedback_per_item"] < 2:
            suggestions.append(
                {"type": "feedback_coverage", "suggestion": "Consider improving feedback tracking"}
            )
