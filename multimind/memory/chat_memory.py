...
class ChatMemory(BufferMemory):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.token_buffer = (
            TokenBufferMemory(max_tokens=max_tokens, token_model=token_model)
            if max_tokens
            else None
        )
        ...
        self.metadata.update(
            {"conversation_id": None, "participants": set(), "topics": set(), "sentiment": None}
        )
    ...
    async def get_messages(
        ...
        elif self.system_prompt and not any(m.get("role") == "system" for m in messages):
    ...
    async def get_messages_by_participant(self, participant: str) -> List[Dict[str, Any]]:
    ...
    async def get_recent_messages(
        self, n: int = 5, role: Optional[str] = None
    ) -> List[Dict[str, Any]]:
    ...
    async def get_messages_in_timeframe(
        self, start_time: datetime, end_time: datetime, role: Optional[str] = None
    ) -> List[Dict[str, Any]]:
