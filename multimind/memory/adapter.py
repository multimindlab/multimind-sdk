...
class AdapterMemory(BaseMemory):
    ...
    def __init__(
        self, input_size: int = 768, adapter_size: int = 64, learning_rate: float = 0.001, **kwargs
    ):
    ...
    async def add_memory(
        ...
        if session_id not in self.session_memories:
            ...
            self.session_adapters[session_id] = AdapterLayer(self.input_size, self.adapter_size)
        ...
        for session_id, memories in self.session_memories.items():
            ...
            for memory in memories:
                similarity = torch.cosine_similarity(
                    adapted_query.unsqueeze(0), memory.unsqueeze(0)
                ).item()
    ...
    async def update_memory(self, memory_id: str, updates: Dict[str, Any]) -> None:
        ...
        if "content" in updates and "session_id" in updates:
            ...
            if session_id in self.session_memories:
                ...
                similarities = [
                    torch.cosine_similarity(adapted_query.unsqueeze(0), memory.unsqueeze(0)).item()
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "sessions": {
                session_id: len(memories) for session_id, memories in self.session_memories.items()
            },
        }
        ...
        if len(memories) > 1:
            ...
            for i in range(len(memories) - 1):
                ...
                self.avg_adaptation_loss = (
                    self.avg_adaptation_loss * (self.total_updates - 1) + loss.item()
                ) / self.total_updates
