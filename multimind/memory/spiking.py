...
class LIFNeuron:
    ...
    def __init__(
        self, threshold: float = 1.0, decay_rate: float = 0.1, refractory_period: float = 0.1
    ):
    ...
    def update(self, input_current: float, current_time: float) -> bool:
        ...
        self.membrane_potential = self.membrane_potential * np.exp(-self.decay_rate) + input_current
...
class STDP:
    ...
    def update(
        self, pre_spike_time: float, post_spike_time: float, pre_id: str, post_id: str
    ) -> None:
        ...
        if dt > 0:  # Pre-before-post: LTP
            ...
            self.weights[(pre_id, post_id)] = min(
                self.weights[(pre_id, post_id)] + dw, self.weight_max
            )
        else:  # Post-before-pre: LTD
            dw = -self.learning_rate * np.exp(dt / self.tau_minus)
            self.weights[(pre_id, post_id)] = max(self.weights[(pre_id, post_id)] + dw, 0.0)
...
class SpikingMemory(BaseMemory):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.stdp = STDP(
            learning_rate=stdp_learning_rate, tau_plus=stdp_tau_plus, tau_minus=stdp_tau_minus
        )
    ...
    async def get_memory(
        self, memory_id: str, current_time: Optional[float] = None
            ...
            if neuron.update(input_current, current_time):
                ...
                for other_id, other_neuron in self.neurons.items():
                    if other_id != neuron_id and other_neuron.spike_history:
                        self.stdp.update(
                            other_neuron.spike_history[-1], current_time, other_id, neuron_id
                        )

        return memory

    async def get_spike_history(
        self, memory_id: str, time_window: Optional[float] = None
    ) -> List[float]:
    ...
    async def get_neuron_stats(self, neuron_id: str) -> Dict[str, Any]:
        ...
        if not spikes:
            return {"total_spikes": 0, "firing_rate": 0.0, "last_spike": None}
    ...
    async def get_synaptic_weights(self, neuron_id: str) -> Dict[Tuple[str, str], float]:
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_neurons": sum(1 for neuron in self.neurons.values() if neuron.spike_history),
        }
