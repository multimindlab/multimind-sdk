...
class WorkingMemory(BaseMemory):
    ...
    async def restore_from_backup(self, backup_index: int = -1) -> None:
        ...
        for item in self.items:
            self.item_embeddings.append(await self.llm.embeddings(item["content"]))
    ...
    async def load(self) -> None:
        ...
        self.last_attention_update = datetime.fromisoformat(
            data.get("last_attention_update", datetime.now().isoformat())
        )
    ...
    async def get_backup_info(self) -> List[Dict[str, Any]]:
        ...
        return [
            {"index": i, "timestamp": backup["timestamp"], "item_count": len(backup["items"])}
    ...
    async def get_working_memory_stats(self) -> Dict[str, Any]:
        ...
        stats["priority_stats"] = {
            "average_priority": (
                sum(self.priority_scores.values()) / len(self.priority_scores)
