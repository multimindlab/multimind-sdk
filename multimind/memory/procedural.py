...
class ProceduralMemory(BaseMemory):
    ...
    async def add_message(self, message: Dict[str, str]) -> None:
        ...
        if procedure:
            ...
            if self.enable_optimization:
                ...
                if (
                    current_time - self.last_optimization
                ).total_seconds() > self.optimization_interval:
    ...
    async def record_execution(
        self, procedure_id: str, success: bool, duration: float, notes: Optional[str] = None
    ) -> None:
        ...
        # Update success rate
        success_count = sum(
            1 for record in self.execution_history[procedure_id] if record["success"]
        )
    ...
    async def _adapt_procedure(self, procedure_id: str, execution_record: Dict[str, Any]) -> None:
    ...
    async def _maintain_procedure_limit(self) -> None:
        ...
        if len(self.procedures) > self.max_procedures:
            ...
            sorted_procedures = sorted(
                self.procedures, key=lambda x: self.procedure_weights[x["id"]]
            )
                ...
                json.dump(
                    {
                        ...
                        "procedure_metadata": {
                            k: {**v, "prerequisites": list(v["prerequisites"])}
    ...
    async def load(self) -> None:
        ...
        if self.storage_path and self.storage_path.exists():
            with open(self.storage_path) as f:
                ...
                self.procedure_metadata = {
                    k: {**v, "prerequisites": set(v["prerequisites"])}
    ...
    async def get_execution_history(
        self, procedure_id: str, min_success_rate: Optional[float] = None
    ) -> List[Dict[str, Any]]:
        ...
        return [
            record
            for record in self.execution_history[procedure_id]
        ...
        stats = {
            ...
            "execution_stats": {
                "total_executions": sum(
                    metadata["execution_count"] for metadata in self.procedure_metadata.values()
                ),
                "average_duration": (
                    sum(
                    ...
                    / len(self.procedure_metadata)
                    if self.procedure_metadata
                    else 0
                ),
            },
            "validation_stats": {"validated": 0, "unvalidated": 0},
            "optimization_stats": {"optimized": 0, "unoptimized": 0},
        }
    ...
    async def get_procedure_suggestions(self) -> List[Dict[str, Any]]:
        ...
        if stats["validation_stats"]["unvalidated"] > len(self.procedures) * 0.5:
            suggestions.append(
                {"type": "validation", "suggestion": "Consider running procedure validation"}
            )

        # Check optimization status
        if stats["optimization_stats"]["unoptimized"] > len(self.procedures) * 0.5:
            suggestions.append(
                {"type": "optimization", "suggestion": "Consider running procedure optimization"}
            )
    ...
    async def _update_procedure_chains(self, procedure_id: str) -> None:
        ...
        for i, other_procedure in enumerate(self.procedures):
            ...
            similarity = self._cosine_similarity(
                self.procedure_embeddings[procedure_idx], self.procedure_embeddings[i]
            )
    ...
    async def _monitor_procedures(self) -> None:
        ...
        for procedure in self.procedures:
            ...
            if execution_records:
                ...
                success_rate = sum(1 for r in execution_records if r["success"]) / len(
                    execution_records
                )
    ...
    async def _update_learning_progress(self, procedure_id: str) -> None:
        ...
        # Update learning progress
        progress = self.learning_rate * success_rate + self.learning_rate * (1 / (1 + avg_duration))

        self.procedure_metadata[procedure_id]["learning_progress"] = min(
            1.0, self.procedure_metadata[procedure_id]["learning_progress"] + progress
        )
    ...
    async def get_learning_history(
        self, procedure_id: str, min_progress: Optional[float] = None
    ) -> List[Dict[str, Any]]:
        ...
        return [
            record
            for record in self.learning_history[procedure_id]
    ...
    async def get_procedure_stats(self) -> Dict[str, Any]:
        ...
        stats["chain_stats"] = {
            ...
            "average_chain_length": (
                sum(len(chain) for chain in self.procedure_chains.values())
                / len(self.procedure_chains)
                if self.procedure_chains
                else 0
            ),
            "max_chain_length": (
                max(len(chain) for chain in self.procedure_chains.values())
                if self.procedure_chains
                else 0
            ),
        }
        ...
        stats["monitoring_stats"] = {
            "average_performance": (
                sum(m["performance"] for m in self.monitoring_metrics.values())
                / len(self.monitoring_metrics)
                if self.monitoring_metrics
                else 0
            ),
            "average_reliability": (
                sum(m["reliability"] for m in self.monitoring_metrics.values())
                / len(self.monitoring_metrics)
                if self.monitoring_metrics
                else 0
            ),
            "average_efficiency": (
                sum(m["efficiency"] for m in self.monitoring_metrics.values())
                / len(self.monitoring_metrics)
                if self.monitoring_metrics
                else 0
            ),
        ...
        stats["learning_stats"] = {
            "average_progress": (
                sum(p["metadata"]["learning_progress"] for p in self.procedures)
                / len(self.procedures)
                if self.procedures
                else 0
            ),
            "procedures_with_progress": sum(
                1 for p in self.procedures if p["metadata"]["learning_progress"] > 0
            ),
        }
