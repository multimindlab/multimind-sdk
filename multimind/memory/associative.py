...
class AssociativeMemory(BaseMemory):
    ...
    async def add_message(self, message: Dict[str, str]) -> None:
        ...
        self.relationships[association_id] = {rel_type: [] for rel_type in self.relationship_types}
    ...
    async def _find_relationships(self, association_id: str) -> None:
        ...
        for i, other_association in enumerate(self.associations):
            ...
            similarity = self._cosine_similarity(
                self.association_embeddings[association_idx], self.association_embeddings[i]
            )

            if similarity >= self.similarity_threshold:
                # Determine relationship type
                relationship_type = await self._determine_relationship_type(
                    association, other_association, similarity
                )

                if relationship_type:
                    # Add bidirectional relationship
                    self.relationships[association_id][relationship_type].append(
                        other_association["id"]
                    )
                    self.relationships[other_association["id"]][relationship_type].append(
                        association_id
                    )

    async def _determine_relationship_type(
        self, assoc1: Dict[str, Any], assoc2: Dict[str, Any], similarity: float
    ) -> Optional[str]:
    ...
    async def _update_patterns(self) -> None:
        ...
        for i, assoc1 in enumerate(self.associations):
            ...
            for j, assoc2 in enumerate(self.associations[i + 1 :], i + 1):
                ...
                similarity = self._cosine_similarity(
                    self.association_embeddings[i], self.association_embeddings[j]
                )
    ...
    async def _update_learning_progress(self, association_id: str) -> None:
        ...
        relationship_count = sum(
            len(relationships) for relationships in self.relationships[association_id].values()
        )
        ...
        association["metadata"]["learning_progress"] = min(
            1.0, association["metadata"]["learning_progress"] + progress
        )
    ...
    async def _maintain_association_limit(self) -> None:
        ...
        if len(self.associations) > self.max_associations:
            ...
            sorted_associations = sorted(
                self.associations, key=lambda x: x["metadata"]["learning_progress"]
            )

            # Remove associations with lowest progress
            associations_to_remove = sorted_associations[
                : len(self.associations) - self.max_associations
            ]
    ...
    async def _remove_association(self, association_id: str) -> None:
        ...
        association_idx = next(
            i for i, a in enumerate(self.associations) if a["id"] == association_id
        )
    ...
    async def get_relationships(
        self, association_id: str, relationship_type: Optional[str] = None
    ) -> Dict[str, List[str]]:
    ...
    async def get_clusters(self, min_size: Optional[int] = None) -> Dict[str, List[str]]:
    ...
    async def get_learning_history(
        self, association_id: str, min_progress: Optional[float] = None
    ) -> List[Dict[str, Any]]:
        ...
        return [
            record
            for record in self.learning_history[association_id]
    ...
    async def get_temporal_relationships(
        ...
        if relationship_type:
            return [
                link
                for link in self.temporal_relationships[association_id]
    ...
    async def get_evolution_history(
        self, association_id: str, min_stage: Optional[int] = None
    ) -> List[Dict[str, Any]]:
        ...
        return [
            record
            for record in self.evolution_history[association_id]
        ...
        stats = {
            ...
            "pattern_stats": {
                ...
                "average_pattern_size": (
                    sum(len(p["associations"]) for p in self.patterns.values()) / len(self.patterns)
                    if self.patterns
                    else 0
                ),
            },
            ...
            "cluster_stats": {
                ...
                "average_cluster_size": (
                    sum(len(cluster) for cluster in self.clusters.values()) / len(self.clusters)
                    if self.clusters
                    else 0
                ),
                "max_cluster_size": (
                    max(len(cluster) for cluster in self.clusters.values()) if self.clusters else 0
                ),
            },
            "learning_stats": {
                "average_progress": (
                    sum(a["metadata"]["learning_progress"] for a in self.associations)
                    / len(self.associations)
                    if self.associations
                    else 0
                ),
                "associations_with_progress": sum(
                    1 for a in self.associations if a["metadata"]["learning_progress"] > 0
                ),
            },
            "temporal_stats": {
                "total_temporal_links": sum(
                    len(links) for links in self.temporal_relationships.values()
                ),
                "average_links_per_association": (
                    sum(len(links) for links in self.temporal_relationships.values())
                    / len(self.temporal_relationships)
            ...
            "confidence_stats": {
                ...
                "high_confidence_associations": sum(
                    1
            ...
            "evolution_stats": {
                "stage_distribution": {
                    stage: sum(
                        1 for a in self.associations if a["metadata"]["evolution_stage"] == stage
                    )
                    for stage in range(4)
                },
                "average_stage": (
                    sum(a["metadata"]["evolution_stage"] for a in self.associations)
                    / len(self.associations)
                    if self.associations
                    else 0
                ),
