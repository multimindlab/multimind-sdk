...
class EpisodicMemory(BaseMemory):
    ...
    async def _find_most_related_episode(self, episode: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    async def get_episode_chain(
        self, episode_id: str, max_depth: Optional[int] = None
    ) -> List[Dict[str, Any]]:
    ...
    async def get_emotional_profile(
        self, episode_id: str, min_intensity: Optional[float] = None
    ) -> Dict[str, float]:
        ...
        for i, episode1 in enumerate(self.episodes):
            ...
            for j, episode2 in enumerate(self.episodes[i + 1 :], i + 1):
                ...
                similarity = self._cosine_similarity(
                    self.episode_embeddings[i], self.episode_embeddings[j]
                )
    ...
    async def _merge_episodes(self, episode_id1: str, episode_id2: str) -> None:
        ...
        episode1["metadata"]["confidence"] = min(
            episode1["metadata"]["confidence"], episode2["metadata"]["confidence"]
        )
        episode1["metadata"]["importance"] = max(
            episode1["metadata"]["importance"], episode2["metadata"]["importance"]
        )
    ...
    async def _maintain_episode_limit(self) -> None:
        ...
        if len(self.episodes) > self.max_episodes:
            ...
            sorted_episodes = sorted(self.episodes, key=lambda x: self.episode_weights[x["id"]])
                ...
                json.dump(
                    {
                        ...
                        "spatial_index": {k: list(v) for k, v in self.spatial_index.items()},
                        "temporal_index": self.temporal_index,
                        "emotional_index": {k: list(v) for k, v in self.emotional_index.items()},
    ...
    async def load(self) -> None:
        ...
        if self.storage_path and self.storage_path.exists():
            with open(self.storage_path) as f:
                ...
                self.episode_embeddings = []
                for episode in self.episodes:
                    self.episode_embeddings.append(await self.llm.embeddings(episode["content"]))
    ...
    async def get_episodes_by_location(
        self, location: str, min_confidence: Optional[float] = None
    ) -> List[Dict[str, Any]]:
    ...
    async def get_episodes_by_emotion(
        self, emotion: str, min_confidence: Optional[float] = None
    ) -> List[Dict[str, Any]]:
    ...
    async def get_episodes_by_date(
        self, date: str, min_confidence: Optional[float] = None
    ) -> List[Dict[str, Any]]:
    ...
    async def get_episode_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "consolidation_stats": {"consolidated": 0, "unconsolidated": 0},
            "chain_stats": {
                "total_chains": len(self.episode_chains),
                "max_chain_length": max(
                    (len(chain) for chain in self.episode_chains.values()), default=0
                ),
                "average_chain_length": (
                    sum(len(chain) for chain in self.episode_chains.values())
                    / len(self.episode_chains)
                    if self.episode_chains
                    else 0
                ),
            },
            "emotional_stats": {
                ...
                "average_intensity": (
                    ...
                    / len(self.emotional_profiles)
                    if self.emotional_profiles
                    else 0
                ),
            },
        }
    ...
    async def get_episode_suggestions(self) -> List[Dict[str, Any]]:
        ...
        if stats["consolidation_stats"]["unconsolidated"] > len(self.episodes) * 0.5:
            suggestions.append(
                {"type": "consolidation", "suggestion": "Consider running episode consolidation"}
            )
