...
class HierarchicalMemory(BaseMemory):
    ...
    async def _categorize_message(self, message: Dict[str, str]) -> tuple[str, str, float]:
    ...
    async def _create_node(self, node_id: str, category: str, parent_id: str) -> None:
        ...
        for i, child1 in enumerate(children):
            for child2 in children[i + 1 :]:
                sim = await self._calculate_similarity(
                    self.node_map[child1]["content"], self.node_map[child2]["content"]
                )
                if sim >= self.similarity_threshold:
                    similarities[(child1, child2)] = sim

        # Merge most similar pairs
        for (child1, child2), _ in sorted(similarities.items(), key=lambda x: x[1], reverse=True):
    ...
    async def _merge_nodes(self, node1_id: str, node2_id: str) -> None:
        ...
        merged_node = {
            ...
            "category_embeddings": (node1["category_embeddings"] + node2["category_embeddings"])
            / 2,
    ...
    def _collect_messages(self, node: Dict[str, Any], messages: List[Dict[str, str]]) -> None:
    ...
    async def _update_node_importance(self, node_id: str) -> None:
        ...
        message_importance = sum(msg.get("importance", 1.0) for msg in node["messages"])
        ...
        parent = self._get_parent_node(node_id)
        if parent:
            parent["importance"] = max(parent["importance"], node["importance"] * 0.8)

    async def _update_category_embeddings(self, node_id: str, content: str) -> None:
            ...
            if not node["category_embeddings"]:
                ...
                for i in range(len(node["category_embeddings"])):
                    node["category_embeddings"][i] = [
                        (1 - self.category_learning_rate) * old + self.category_learning_rate * new
                        for old, new in zip(node["category_embeddings"][i], new_embedding)
                    ]
        except Exception as e:
            logger.error(f"Error updating category embeddings: {e}")

    async def query_hierarchy(
        self, query: str, max_results: int = 5, min_importance: float = 0.3
    ) -> List[Dict[str, Any]]:
    ...
    async def get_important_nodes(
        self, min_importance: float = 0.5, max_depth: Optional[int] = None
    ) -> List[Dict[str, Any]]:
        ...
        return sorted(important_nodes, key=lambda x: x["importance"], reverse=True)

    async def get_node_relationships(self, node_id: str, max_distance: int = 2) -> Dict[str, Any]:
    ...
    async def get_semantic_analysis(self, node_id: str) -> Dict[str, Any]:
    ...
    async def _get_related_nodes(self, node_id: str, max_related: int = 5) -> List[Dict[str, Any]]:
                        ...
                        related_nodes.append(
                            {
                                ...
                                "shared_tags": list(
                                    node["semantic_tags"] & related_node["semantic_tags"]
                                ),
        ...
        related_nodes.sort(key=lambda x: (len(x["shared_tags"]), x["importance"]), reverse=True)

        return related_nodes[:max_related]

    async def _get_semantic_cluster(self, node_id: str) -> Dict[str, Any]:
        ...
        if related_nodes:
            ...
            if cluster["cluster_center"]:
                ...
                for emb in embeddings:
                    similarity = self._cosine_similarity(cluster["cluster_center"], emb)
    ...
    async def get_lifecycle_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "lifecycle_states": {"active": 0, "inactive": 0, "archived": 0},
            "age_distribution": {
                "new": 0,  # < 1 day
                "young": 0,  # 1-7 days
                "mature": 0,  # 7-30 days
    ...
    async def get_hierarchy_stats(self) -> Dict[str, Any]:
        ...
        for node_id, node in self.node_map.items():
            ...
            stats["message_distribution"][depth] = stats["message_distribution"].get(
                depth, 0
            ) + len(node["messages"])
    ...
    async def get_node_suggestions(self) -> List[Dict[str, Any]]:
        ...
        if lifecycle_stats["lifecycle_states"]["archived"] > len(self.node_map) * 0.3:
            suggestions.append(
                {"type": "lifecycle_cleanup", "suggestion": "Consider cleaning up archived nodes"}
            )
    ...
    async def _update_semantic_analysis(self, node_id: str, content: str) -> None:
