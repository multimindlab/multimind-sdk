...
class EmotionalMemory(BaseMemory):
    ...
    def __init__(
        ...
        self.emotion_weights = emotion_weights or {"valence": 0.4, "arousal": 0.3, "dominance": 0.3}
    ...
    async def add_message(self, message: Dict[str, str]) -> None:
        ...
        self.relationships[state_id] = {rel_type: [] for rel_type in self.relationship_types}
    ...
    async def _determine_relationship_type(
        self, state1: Dict[str, Any], state2: Dict[str, Any], similarity: float
    ) -> Optional[str]:
            ...
            for state_id in self.relationships:
                ...
                if len(cluster) >= self.min_cluster_size:
                    ...
                    for state_id in cluster:
                        self.states[
                            self.states.index(next(s for s in self.states if s["id"] == state_id))
    ...
    async def _update_evolution(self, state_id: str) -> None:
        ...
        relationship_count = sum(
            len(relationships) for relationships in self.relationships[state_id].values()
        )
    ...
    async def get_relationships(
        self, state_id: str, relationship_type: Optional[str] = None
    ) -> Dict[str, List[str]]:
        ...
        if relationship_type:
            return {relationship_type: self.relationships[state_id].get(relationship_type, [])}
    ...
    async def get_evolution_history(
        self, state_id: str, min_stage: Optional[int] = None
    ) -> List[Dict[str, Any]]:
        ...
        return [
            record for record in self.evolution_history[state_id] if record["stage"] >= min_stage
        ]
    ...
    async def get_emotional_memory_stats(self) -> Dict[str, Any]:
        ...
        stats["relationship_stats"] = {
            ...
            "relationship_types": {
                rel_type: sum(
                    1 for relationships in self.relationships.values() if relationships[rel_type]
                )
        ...
        stats["evolution_stats"] = {
            ...
            "average_stage": (
                sum(s["metadata"]["evolution_stage"] for s in self.states) / len(self.states)
                if self.states
                else 0
            ),
        }
