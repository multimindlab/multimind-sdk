...
class ContextTransferManager:
    ...
    def extract_context(
        self, messages: List[Dict], last_n: int = 5, smart_extraction: bool = True
    ) -> List[Dict]:
        """
    ...
    def _smart_extract_context(self, messages: List[Dict], last_n: int) -> List[Dict]:
        ...
        if important_context:
            ...
            logger.info(
                f"Smart extraction: {len(context_to_include)} important + {len(combined) - len(context_to_include)} recent messages"
            )
            return combined
    ...
    def summarize_context(
        self, messages: List[Dict], model: str = "gpt-3.5", summary_type: str = "concise"
    ) -> str:
        """
    ...
    def _create_structured_summary(self, messages: List[Dict]) -> str:
        ...
        logger.info(
            f"Generated structured summary with {len(user_messages)} user and {len(assistant_messages)} assistant messages"
        )
        return summary

    def load_conversation_from_file(self, file_path: str, format_type: str = "auto") -> List[Dict]:
        """
    ...
    def _load_json_conversation(self, file_path: str) -> List[Dict]:
        ...
        if isinstance(data, list):
            ...
            raise ValueError(
                "Invalid JSON structure. Expected list of messages or dict with 'messages' key."
            )
    ...
    def save_formatted_prompt(
        self, content: str, output_file: str, format_type: str = "txt"
    ) -> None:
        """
...
"""
    ...
    def _format_for_target_model(
        self, target_model: str, summary: str, source_model: str, **kwargs
    ) -> str:
        """
