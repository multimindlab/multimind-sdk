...
class ContextTransferAPI:
    ...
    def transfer_context_api(
        ...
        try:
            ...
            extracted_messages = self.manager.extract_context(
                messages, default_options["last_n"], default_options["smart_extraction"]
            )

            # Generate summary
            if default_options["include_summary"]:
                summary = self.manager.summarize_context(
                    extracted_messages, summary_type=default_options["summary_type"]
                )
            else:
                summary = self.manager.summarize_context(
                    extracted_messages[-1:], summary_type="concise"
                )
    ...
    def get_supported_models(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "metadata": {"generated_at": datetime.now().isoformat(), "api_version": "2.0"},
            }
        except Exception as e:
    ...
    def _generate_recommendations(self, analysis: Dict[str, Any]) -> List[str]:
        ...
        if analysis["assistant_messages"] == 0:
            recommendations.append(
                "No assistant messages found - ensure conversation has AI responses"
            )

        if analysis["unknown_messages"] > 0:
            recommendations.append(
                f"Found {analysis['unknown_messages']} messages with unknown roles"
            )
        ...
        if analysis["total_messages"] > 20:
            recommendations.append(
                "Large conversation detected - consider using smart extraction and detailed summary"
            )

        if not analysis["has_system_context"]:
            recommendations.append(
                "No system context found - consider adding system messages for better context"
            )

        return recommendations

    def batch_transfer(self, transfers: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
    ...
    def create_chrome_extension_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "metadata": {"generated_at": datetime.now().isoformat(), "sdk_version": "2.0.0"},
        }


# Convenience functions for easy integration
def quick_transfer(
    source_model: str, target_model: str, conversation_data: Union[str, List[Dict], Dict], **kwargs
) -> str:
    """
