...
class HybridRetriever:
    ...
    async def _rerank(
        self, query: str, results: List[RetrievalResult], **kwargs
    ) -> List[RetrievalResult]:
...
class QueryDecomposer:
    ...
    async def decompose(self, query: str, **kwargs) -> List[Dict[str, Any]]:
        """
    ...
    def _parse_decomposition(self, response: str) -> List[Dict[str, Any]]:
        ...
        return [{"sub_query": response, "type": QueryType.FACTUAL.value, "requirements": {}}]
...
class MultiVectorRetriever:
    ...
    async def add_aspect_embeddings(
        self, documents: List[str], aspects: List[str], **kwargs
    ) -> None:
        """
        ...
        for aspect in aspects:
            ...
            aspect_prompts = [f"Extract the {aspect} information from: {doc}" for doc in documents]
    ...
    async def retrieve(
        self, query: str, aspects: Optional[List[str]] = None, k: int = 3, **kwargs
    ) -> List[Dict[str, Any]]:
        """
        ...
        aspect_queries = [
            f"Find documents relevant to {aspect} regarding: {query}" for aspect in aspects
        ]
        ...
        return [
            {
                ...
                "aspect_scores": {
                    aspect: float(scores[idx]) for aspect, scores in zip(aspects, all_scores)
                },
            }
