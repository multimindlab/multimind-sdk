...
class LSTMModelClient(ModelClient):
    def __init__(self, model_path, tokenizer):
        # weights_only=False because these clients load *whole pickled
        # modules* (the user's own trained model objects). PyTorch >=2.6
        # defaults to weights_only=True which rejects arbitrary class
        # instances; opt back in explicitly since callers control the file.
...
class RNNModelClient(ModelClient):
    def __init__(self, model_path, tokenizer):
        # See LSTMModelClient.__init__ — same trusted-load rationale.
        self.model = torch.load(model_path, weights_only=False)
        self.model.eval()
        self.tokenizer = tokenizer
...
class MoEModelClient(ModelClient):
    def __init__(self, expert_clients: Dict[str, ModelClient], router_fn: Callable[[str], str]):
        self.expert_clients = (
            expert_clients  # e.g., {"rnn": LSTMModelClient(), "mamba": MambaClient()}
        )
...
class DynamicMoEModelClient(MoEModelClient):
    ...
    def __init__(
        self, expert_clients: Dict[str, ModelClient], router_fn: Callable[[str, dict], str]
    ):
    ...
    def generate(self, prompt: str, **kwargs):
        ...
        avg_latencies = {
            k: (sum(v["latency"]) / len(v["latency"]) if v["latency"] else float("inf"))
            for k, v in self.metrics.items()
        }
        # Call router_fn with prompt and metrics
        selected_expert = self.router_fn(
            prompt, {"input_length": input_length, "avg_latencies": avg_latencies}
        )
        client = self.expert_clients[selected_expert]
        start = time.time()
