...
class JiraIntegrationHandler(IntegrationHandler, AsyncContextManager):
    ...
    async def create_issue(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with self.session.post(f"{self.api_base}/issue", json=payload) as response:
            result = await response.json()

            if response.status != 201:
                raise Exception(
                    f"Failed to create Jira issue: {result.get('message', 'Unknown error')}"
                )

            return {"issue_key": result["key"], "issue_id": result["id"], "self": result["self"]}
    ...
    async def update_issue(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with self.session.put(f"{self.api_base}/issue/{issue_key}", json=payload) as response:
            if response.status != 204:
                result = await response.json()
                raise Exception(
                    f"Failed to update Jira issue: {result.get('message', 'Unknown error')}"
                )
    ...
    async def get_issue(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with self.session.get(f"{self.api_base}/issue/{issue_key}") as response:
            result = await response.json()

            if response.status != 200:
                raise Exception(
                    f"Failed to get Jira issue: {result.get('message', 'Unknown error')}"
                )

            return {"key": result["key"], "fields": result["fields"], "self": result["self"]}
    ...
    async def search_issues(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with self.session.get(f"{self.api_base}/search", params=params) as response:
            result = await response.json()

            if response.status != 200:
                raise Exception(
                    f"Failed to search Jira issues: {result.get('message', 'Unknown error')}"
                )

            return {
                "total": result["total"],
                "issues": [
                    {"key": issue["key"], "fields": issue["fields"], "self": issue["self"]}
