...
class DiscordIntegrationHandler(IntegrationHandler, AsyncContextManager):
    ...
    async def send_message(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with self.session.post(
            f"{self.api_base}/channels/{channel_id}/messages", json=payload
        ) as response:
            result = await response.json()

            if response.status != 200:
                raise Exception(
                    f"Failed to send Discord message: {result.get('message', 'Unknown error')}"
                )
    ...
    async def create_channel(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with self.session.post(
            f"{self.api_base}/guilds/{guild_id}/channels", json=payload
        ) as response:
            result = await response.json()

            if response.status != 200:
                raise Exception(
                    f"Failed to create Discord channel: {result.get('message', 'Unknown error')}"
                )
    ...
    async def list_channels(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with self.session.get(f"{self.api_base}/guilds/{guild_id}/channels") as response:
            ...
            if response.status != 200:
                raise Exception(
                    f"Failed to list Discord channels: {result.get('message', 'Unknown error')}"
                )
    ...
    async def create_role(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with self.session.post(
            f"{self.api_base}/guilds/{guild_id}/roles", json=payload
        ) as response:
            result = await response.json()

            if response.status != 200:
                raise Exception(
                    f"Failed to create Discord role: {result.get('message', 'Unknown error')}"
                )
