...
class AsyncVectorStore(Protocol):
    ...
    async def add(
        self, vectors: List[List[float]], documents: List[str], metadata: List[Dict[str, Any]]
    ) -> None: ...
...
class BaseRAG(ABC):
    ...
    async def optimize_context(
        self, query: str, context: List[Dict[str, Any]], max_tokens: int, **kwargs
    ) -> List[Dict[str, Any]]:
    ...
    @abstractmethod
    async def validate_documents(self, documents: List[str], **kwargs) -> List[Dict[str, Any]]:
        """Validate documents before processing."""
        pass

    @abstractmethod
    async def reindex(self, documents: Optional[List[str]] = None, **kwargs) -> None:
