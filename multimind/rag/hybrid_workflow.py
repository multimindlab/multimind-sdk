...
from ..core.provider import GenerationResult
...
class HybridWorkflow:
    ...
    async def add_document(
        ...
        if generate_embeddings:
            embedding_result = await self.router.route(
                TaskType.EMBEDDINGS, content, model="text-embedding-ada-002"
            )
    ...
    async def add_image_document(
        ...
        image_doc = ImageDocument(image_data=image_data, metadata=metadata or {})

        if analyze_image:
            analysis_result = await self.router.route(
                TaskType.IMAGE_ANALYSIS, image_data, prompt="Analyze this image in detail."
            )
            image_doc.analysis = {
    ...
    async def process_with_rag(
        self, query: str, context_id: str, task_type: TaskType = TaskType.TEXT_GENERATION, **kwargs
    ) -> GenerationResult:
        ...
        query_embedding_result = await self.router.route(
            TaskType.EMBEDDINGS, query, model="text-embedding-ada-002"
        )
        query_embeddings = query_embedding_result.embeddings

        # Find relevant documents
        relevant_docs = await self._find_relevant_documents(
            query_embeddings, context.documents, top_k=kwargs.pop("top_k", 3)
        )
        ...
        result = await self.router.route(
            task_type,
            {
                ...
                "metadata": {"relevant_docs": [doc.metadata for doc in relevant_docs]},
            },
            **kwargs,
        )
    ...
    async def process_vision_language(
        self, image_data: bytes, prompt: str, context_id: str, **kwargs
    ) -> GenerationResult:
        ...
        relevant_images = await self._find_relevant_images(
            prompt, context.image_documents, top_k=kwargs.pop("top_k", 1)
        )
        ...
        result = await self.router.route(
            TaskType.TEXT_GENERATION,
            {
                ...
                "metadata": {"relevant_images": [img.metadata for img in relevant_images]},
            },
            **kwargs,
        )
    ...
    async def process_hybrid(
        self, query: str, image_data: Optional[bytes] = None, context_id: str = "default", **kwargs
    ) -> GenerationResult:
        ...
        if image_data:
            analysis_result = await self.router.route(
                TaskType.IMAGE_ANALYSIS, image_data, prompt=query
            )
            image_analysis = {
                "objects": analysis_result.objects,
                "captions": analysis_result.captions,
        ...
        relevant_images = await self._find_relevant_images(
            query, context.image_documents, top_k=kwargs.pop("top_k", 1)
        )
    ...
    async def _find_relevant_documents(
        self, query_embeddings: List[float], documents: List[Document], top_k: int = 3
    ) -> List[Document]:
    ...
    async def _find_relevant_images(
        self, query: str, images: List[ImageDocument], top_k: int = 1
    ) -> List[ImageDocument]:
        ...
        for img in images:
            if img.analysis and img.analysis.get("text"):
                ...
                analysis_embedding_result = await self.router.route(
                    TaskType.EMBEDDINGS, img.analysis["text"], model="text-embedding-ada-002"
                )
                similarity = self._cosine_similarity(
                    query_embeddings, analysis_embedding_result.embeddings
                )
