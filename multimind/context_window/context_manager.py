...
from ..embeddings.embedding import EmbeddingConfig, EmbeddingModel
...
class ContextManager:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if TIKTOKEN_AVAILABLE and tiktoken is not None:
            ...
            self.logger.warning("tiktoken is not available; using fallback tokenizer (word-based).")
            self.tokenizer = _FallbackTokenizer()

        # Initialize context window
        self.window = ContextWindow(
            chunks=[], metadata={}, total_tokens=0, last_updated=datetime.now().timestamp()
        )

        # Initialize cache
        self.cache = {"embeddings": {}, "relevance_scores": {}, "compressed_chunks": {}}
    ...
    async def add_to_context(
        ...
        """
        # Create chunks
        chunks = await self._create_chunks(content, metadata or {})
    ...
    async def _create_chunks(self, content: str, metadata: Dict[str, Any]) -> List[ContextChunk]:
    ...
    async def _prune_window(self) -> None:
        ...
        self.window.chunks.sort(key=lambda x: x.relevance_score, reverse=True)
        ...
        while self.window.total_tokens > self.config.max_tokens:
            ...
            if self.vector_store:
                await self.vector_store.delete_vectors([f"chunk_{len(self.window.chunks) + 1}"])
    ...
    async def compress_context(
        self, strategy: str = CompressionStrategy.SEMANTIC.value, **kwargs
    ) -> None:
        """
    ...
    async def _semantic_compression(self) -> None:
        ...
        for chunk in self.window.chunks:
            if not current_group:
                ...
                similarity = await self._calculate_similarity(chunk, current_group[0])
        ...
        self.window.total_tokens = sum(chunk.tokens for chunk in compressed_chunks)
    ...
    async def _combine_chunks(self, chunks: List[ContextChunk]) -> ContextChunk:
        """Combine similar chunks."""
        if not self.llm:
        ...
        return ContextChunk(
            ...
            embedding=None,
            relevance_score=max(chunk.relevance_score for chunk in chunks),
            timestamp=datetime.now().timestamp(),
        )
    ...
    async def load_context(
        ...
        # Create instance
        manager = cls(
            embedding_model=embedding_model, llm=llm, vector_store=vector_store, config=config
        )
        ...
        if vector_store and (path / "vector_store").exists():
            manager.vector_store = await VectorStore.load(
                str(path / "vector_store"), vector_store.config
            )
