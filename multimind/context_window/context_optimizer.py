...
class PromptGenerator:
    ...
    def __init__(
        self, model: BaseLLM, default_template: PromptTemplate = PromptTemplate.STANDARD, **kwargs
    ):
        self.model = model
    ...
    async def generate_prompt(
        ...
        **kwargs,
        ...
        prompt = template_str.format(
            context=context_text, query=query, few_shot_examples=few_shot_text
        )

        return prompt

    async def select_few_shot_examples(
        self, query: str, examples: List[Dict[str, str]], k: int = 3, **kwargs
    ) -> List[Dict[str, str]]:
        """
        ...
        **kwargs,
        ...
        if few_shot_examples:
            selected_examples = await self.prompt_generator.select_few_shot_examples(
                query, few_shot_examples, **kwargs
            )

        # Generate prompt
        prompt = await self.prompt_generator.generate_prompt(
            query, optimized_context, template, selected_examples, **kwargs
        )
