...
from .multitask_peft import TaskConfig, UniPELTPlusMethod
...
class MAMLLearner:
    ...
    def __init__(self, model: nn.Module, maml_config: Optional[Dict[str, Any]] = None):
        self.model = model
        ...
        self.meta_optimizer = Adam(self.model.parameters(), lr=self.maml_config["outer_lr"])
    ...
    def _inner_loop(
        self, task_data: Dict[str, torch.Tensor], clone: nn.Module
    ) -> Tuple[nn.Module, float]:
        """Perform inner loop adaptation."""
        # Initialize task-specific optimizer
        task_optimizer = Adam(clone.parameters(), lr=self.maml_config["inner_lr"])

        # Split into support and query sets
        support_data = {k: v[: self.maml_config["k_shot"]] for k, v in task_data.items()}
        query_data = {k: v[self.maml_config["k_shot"] :] for k, v in task_data.items()}
    ...
    def meta_train(
        ...
        for episode in range(self.maml_config["n_episodes"]):
            ...
            if (episode + 1) % 10 == 0:
                logger.info(
                    f"MAML Episode {episode + 1}/{self.maml_config['n_episodes']}, "
                    f"Meta-loss: {meta_loss.item():.4f}"
                )

    def adapt_to_task(
        self, support_data: Dict[str, torch.Tensor], query_data: Dict[str, torch.Tensor]
...
class ReptileLearner:
    ...
    def __init__(self, model: nn.Module, reptile_config: Optional[Dict[str, Any]] = None):
        self.model = model
        ...
        # Initialize meta-optimizer
        self.meta_optimizer = Adam(self.model.parameters(), lr=self.reptile_config["outer_lr"])

    def _clone_model(self) -> nn.Module:
        """Create a clone of the model for inner loop updates."""
        return type(self.model)(**self.model.config.to_dict())

    def _inner_loop(self, task_data: Dict[str, torch.Tensor], clone: nn.Module) -> nn.Module:
        """Perform inner loop adaptation."""
        # Initialize task-specific optimizer
        task_optimizer = Adam(clone.parameters(), lr=self.reptile_config["inner_lr"])

        # Split into support and query sets
        support_data = {k: v[: self.reptile_config["k_shot"]] for k, v in task_data.items()}
        ...
        for episode in range(self.reptile_config["n_episodes"]):
            ...
            for task in meta_batch:
                ...
                for (name, param), (_, adapted_param) in zip(
                    self.model.named_parameters(), adapted_model.named_parameters()
                ):
...
class FewShotMetaTuner(MetaOptimizedMultiTaskTuner):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.few_shot_learner = FewShotLearner(
            model=self.model, few_shot_config=few_shot_config, strategy=few_shot_strategy
        )
    ...
    def train(
        ...
        **kwargs,
        ...
        if few_shot_tasks:
            ...
            self.few_shot_learner.meta_train(train_tasks=few_shot_tasks, val_tasks=eval_datasets)
    ...
    def adapt_to_new_task(
        ...
        return {"accuracy": accuracy, "metrics": metrics, "adapted_model": self.model}
...
class TransferLearner:
    ...
    def __init__(self, model: nn.Module, transfer_config: Optional[Dict[str, Any]] = None):
        self.model = model
    ...
    def compute_layer_similarity(
        self, source_features: Dict[str, torch.Tensor], target_features: Dict[str, torch.Tensor]
    ) -> Dict[str, float]:
        """Compute similarity between source and target layers."""
        similarities = {}

        for layer_name in source_features:
            if layer_name in target_features:
                ...
                similarity = F.cosine_similarity(source_flat, target_flat).item()

                similarities[layer_name] = similarity

        return similarities

    def select_transfer_layers(self, similarities: Dict[str, float]) -> List[str]:
    ...
    def adapt_to_target(
        self, target_data: Dict[str, torch.Tensor], selected_layers: List[str]
    ) -> Dict[str, Any]:
        ...
        scheduler = LambdaLR(
            optimizer, lambda step: min(1.0, step / self.transfer_config["warmup_steps"])
        )
...
class TransferMetaTuner(MetaOptimizedMultiTaskTuner):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.transfer_learner = TransferLearner(model=self.model, transfer_config=transfer_config)
        ...
        **kwargs,
        ...
        if source_tasks:
            ...
            transfer_results = self.transfer_learner.adapt_to_target(
                target_data=train_datasets, selected_layers=selected_layers
            )
