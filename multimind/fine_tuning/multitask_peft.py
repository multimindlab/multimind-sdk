...
from .adaptive_peft import AdaptiveUniPELTPlusTuner, DynamicComponentWeighting, UniPELTPlusMethod
...
class MultiTaskMethodSelector:
    ...
    def update_task_performance(
        self, task_name: str, method: UniPELTPlusMethod, metrics: Dict[str, float]
    ) -> None:
    ...
    def get_method_task_importance(self, method: UniPELTPlusMethod, task_name: str) -> float:
        ...
        if task_name not in self.method_task_importance[method]:
            ...
            if (
                task_name in self.task_method_performance
                and method in self.task_method_performance[task_name]
            ):
    ...
    def select_methods_for_tasks(
        self, model_size: int, active_tasks: List[str]
        ...
        for task_name in active_tasks:
            ...
            for method in sorted_methods:
                ...
                if (
                    ...
                    > self.resource_constraints["max_trainable_params"]
                    or total_memory + usage["memory"] > self.resource_constraints["max_memory_gb"]
                    or total_time + usage["time"]
                    > self.resource_constraints["max_training_time_hours"]
                ):
                    continue
...
class CrossModelTransfer:
    ...
    def __init__(
        self, source_model: str, target_model: str, transfer_config: Optional[Dict[str, Any]] = None
    ):
    ...
    def analyze_model_similarity(
        self, source_weights: Dict[str, torch.Tensor], target_weights: Dict[str, torch.Tensor]
    ) -> Dict[str, float]:
            ...
            if name in target_weights:
                ...
                if source_param.shape == target_param.shape:
                    similarity = F.cosine_similarity(
                        source_param.view(1, -1), target_param.view(1, -1)
                    ).item()
    ...
    def transfer_method_weights(
        ...
        similarities = self.analyze_model_similarity(
            source_weights, {name: param for name, param in target_model.named_parameters()}
        )
        ...
        if self.transfer_config["transfer_strategy"] == "selective":
            ...
            for name, similarity in similarities.items():
                if similarity >= self.transfer_config["similarity_threshold"]:
                    ...
                    with torch.no_grad():
                        target_param.data = (
                            1 - self.transfer_config["adaptation_rate"]
                        ) * target_param.data + self.transfer_config[
                            "adaptation_rate"
                        ] * source_param.data
        else:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.task_weights = DynamicComponentWeighting(
            num_components=len(tasks), initial_weights=[task.importance for task in tasks]
        )
                ...
                **kwargs,
                    ...
                    if task_name in self.tuner.tasks:
                        for method in self.tuner.methods:
                            self.tuner.task_selector.update_task_performance(
                                task_name=task_name, method=method, metrics=task_metrics
                            )
                ...
                for task_name in train_datasets:
                    ...
                    if set(new_methods) != set(self.tuner.methods):
                        logger.info(
                            f"Adapting methods for {task_name}: {[m.value for m in new_methods]}"
                        )
...
class CrossModelUniPELTPlusTuner(AdaptiveUniPELTPlusTuner):
    ...
    def _prepare_model(self) -> None:
        ...
        for method, weights in source_weights.items():
            self.transfer.transfer_method_weights(
                source_weights=weights, target_model=self.model, method=method
            )
    ...
    def _load_method_weights(self, method: UniPELTPlusMethod) -> Optional[Dict[str, torch.Tensor]]:
        ...
        try:
            source_model = PeftModel.from_pretrained(self.transfer.source_model, method.value)
            return {
