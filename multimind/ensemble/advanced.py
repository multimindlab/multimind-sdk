...
class AdvancedEnsemble:
    ...
    async def _parallel_voting(
        ...
        **kwargs,
        ...
        evaluations = await asyncio.gather(
            *[self._evaluate_with_llm(result, task_type, **kwargs) for result in results]
        )
    ...
    async def _majority_voting(
        ...
        **kwargs,
        ...
        if embedder is not None:
            ...
            for i, emb in enumerate(embeddings):
                ...
                for j in range(i + 1, len(embeddings)):
                    ...
                    sim = torch.nn.functional.cosine_similarity(
                        emb, embeddings[j], dim=0, eps=1e-6
                    ).item()
            ...
            provider_votes = {
                self._get_provider_name(r): 1.0 if idx in largest_group else 0.0
            ...
            total_votes = len(results)
            explanation = (
                f"Selected result with {vote_count}/{total_votes} votes (string equality fallback)"
            )
            provider_votes = {self._get_provider_name(r): 1.0 for r in results}
        return EnsembleResult(
            result=best_result,
            confidence=ConfidenceScore(score=vote_count / total_votes, explanation=explanation),
    ...
    async def _rank_based(
        ...
        **kwargs,
        ...
        rankings = await asyncio.gather(
            *[self._get_provider_ranking(result, task_type, **kwargs) for result in results]
        )
        ...
        if total_score == 0 or not borda_scores:
            ...
            normalized_scores = (
                {k: 1.0 / len(borda_scores) for k in borda_scores} if borda_scores else {}
            )
            ...
            if not borda_scores:
                ...
                best_result = next(
                    r for r in results if self._get_provider_name(r) == best_provider
                )
        else:
    ...
    def _extract_result_content(
        self, result: Union[GenerationResult, EmbeddingResult, ImageAnalysisResult]
    ) -> str:
    ...
    async def _evaluate_confidence(
        ...
        **kwargs,
        ...
        eval_provider = evaluation_providers.get(
            provider_name, kwargs.get("evaluation_provider", provider_name)
        )
        ...
        return ConfidenceScore(
            score=score, explanation=explanation, metadata={"raw_evaluation": eval_content}
        )
    ...
    def tune_weights_with_optuna(self, results, task_type, eval_fn, n_trials=30):
        ...
        if not OPTUNA_AVAILABLE:
            raise ImportError(
                "Optuna is required for hyperparameter tuning. Please install optuna."
            )
        ...
        def objective(trial):
            ...
            ensemble_result = loop.run_until_complete(
                self._weighted_voting(results, weights=weights, use_adaptive_weights=False)
            )
