...
if TORCH_AVAILABLE:
    ...
    class AdvancedMoELayer(MoELayer):
        ...
        def _compute_dynamic_capacity(self, batch_size: int) -> torch.Tensor:
            ...
            target_capacity = torch.clamp(
                usage_ratio * batch_size, min=self.min_expert_capacity, max=self.max_expert_capacity
            )

            # Smooth capacity updates
            self.current_capacity = 0.9 * self.current_capacity + 0.1 * target_capacity

            return self.current_capacity

        def _compute_specialization_weights(self, x: torch.Tensor) -> torch.Tensor:
            ...
            similarity = F.cosine_similarity(
                x.unsqueeze(1), self.expert_embeddings.unsqueeze(0), dim=-1
            )

            # Combine with router weights
            router_weights = self.specialization_router(x)
            combined_weights = F.softmax(similarity + router_weights, dim=-1)
        ...
        def _prune_experts(self) -> None:
            ...
            self.expert_importance = 0.9 * self.expert_importance + 0.1 * (
                self.expert_usage / (self.expert_usage.sum() + 1e-6)
            )
        ...
        def _apply_gradient_checkpointing(self, expert_idx: int, x: torch.Tensor) -> torch.Tensor:
            ...
            return torch.utils.checkpoint.checkpoint(
                create_custom_forward(self.experts[expert_idx]), x
            )
        ...
        def forward(
            ...
            for i in range(self.k):
                expert_counts.scatter_add_(
                    0, top_k_indices[:, i], torch.ones_like(top_k_indices[:, i], dtype=torch.float)
                )
    ...
    class MoEFactory:
        ...
        @staticmethod
        def create_expert(expert_type: str, expert_id: str, **kwargs):
        ...
        @staticmethod
        def create_router(router_type: str, experts: Dict[str, Any], **kwargs):
