...
from .moe import Expert, MoEBase
...
if TORCH_AVAILABLE:
    ...
    class UnifiedMoE(nn.Module):
        ...
        async def process(
            self, input_data: Union[torch.Tensor, Dict[str, Any]], return_aux_loss: bool = False
        ) -> Dict[str, Any]:
            """
        ...
        async def _process_neural(
            self, input_data: torch.Tensor, return_aux_loss: bool
        ) -> Dict[str, Any]:
            ...
            return {"output": output, "aux_loss": aux_loss, "metrics": self.metrics}

        async def _process_modality(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        def load_checkpoint(cls, path: str) -> "UnifiedMoE":
            ...
            model = cls(mode=checkpoint["mode"], config=checkpoint["config"])
            model.load_state_dict(checkpoint["model_state_dict"])
            model.metrics = checkpoint["metrics"]
            return model

        def add_expert(self, expert_id: str, expert: Expert, modality: Optional[str] = None):
